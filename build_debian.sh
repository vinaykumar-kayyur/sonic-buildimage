...
install_kubernetes () {
    ...
    sudo LANG=C chroot $FILESYSTEM_ROOT apt-get update
    sudo LANG=C chroot $FILESYSTEM_ROOT apt-get -y install kubelet=${ver}-00
    sudo LANG=C chroot $FILESYSTEM_ROOT apt-get -y install kubectl=${ver}-00
    sudo LANG=C chroot $FILESYSTEM_ROOT apt-get -y install kubeadm=${ver}-00
}
...
then
    ...
    sudo LANG=C chroot $FILESYSTEM_ROOT apt-get update
    sudo LANG=C chroot $FILESYSTEM_ROOT apt-get -y install hyperv-daemons gnupg libxml2-utils
    ...
    sudo LANG=C chroot $FILESYSTEM_ROOT rm - /tmp/cri-dockerd.deb
else

...
class Fan(FanBase):
    ...
    def __init__(self, fan_tray_index, fan_index=0):
        ...
        self.redstone_xp_fan_gpio = [
            {'base': self.__get_gpio_base()},
            {'prs': 11, 'dir': 16, 'color': {'red': 31, 'green': 32}},  # 1
            {'prs': 10, 'dir': 15, 'color': {'red': 29, 'green': 30}},  # 2
            {'prs': 14, 'dir': 19, 'color': {'red': 37, 'green': 38}},  # 3
            {'prs': 12, 'dir': 17, 'color': {'red': 33, 'green': 34}},  # 4
        ]
    ...
    def __get_gpio_value(self, pinnum):
        gpio_base = self.redstone_xp_fan_gpio[0]['base']
    ...
    def get_direction(self):
        ...
        if not self.is_psu_fan:
            raw = self.__get_gpio_value(
                self.redstone_xp_fan_gpio[self.fan_tray_index + 1]['dir'])
    ...
    def get_speed(self):
        ...
        if self.is_psu_fan:
            ...
            fan_speed_sysfs_path = self.__search_file_by_name(self.psu_hwmon_path, fan_speed_sysfs_name)
            fan_speed_rpm = self._api_helper.read_txt_file(fan_speed_sysfs_path) or 0
            speed = 100 if int(fan_speed_rpm) > 0 else 0
    ...
    def set_status_led(self, color):
        ...
        s1_gpio = self.redstone_xp_fan_gpio[self.fan_tray_index + 1]['color']['red']
        s2_gpio = self.redstone_xp_fan_gpio[self.fan_tray_index + 1]['color']['green']
    ...
    def get_status_led(self):
        ...
        s1 = self.__get_gpio_value(
            self.redstone_xp_fan_gpio[self.fan_tray_index + 1]['color']['red'])
        s2 = self.__get_gpio_value(
            self.redstone_xp_fan_gpio[self.fan_tray_index + 1]['color']['green'])
    ...
    def get_presence(self):
        ...
        present_str = self.__get_gpio_value(
            self.redstone_xp_fan_gpio[self.fan_tray_index + 1]['prs'])

...
class Sfp(SfpBase):
    ...
    def __init__(self, sfp_index=0,conf=None):
        ...
        self.qsfp_cable_length_tup = ('Length(km)', 'Length OM3(2m)',
    ...
    def _get_eeprom_path(self):
        ...
        argument = port_to_i2c_mapping[self.sfp_index].split(',')
        port_eeprom_path = eeprom_path.format(*argument)

        return port_eeprom_path

    def _get_type(self):
        """
        Retrieves the human-readable Type of a sfp by 1-based index

        Returns:
        :param index: An integer, 1-based index of the sfp of which to query status
        :return: String,
            A string representing the type of the sfp. 
        """
        f_name = inspect.stack()[0][3]
        config = self._config.get(f_name)
        
        return self._api_common.get_output(self.sfp_index, config, Common.NULL_VAL)
    ...
    def get_presence(self):
        ...
        output = self._api_common.get_output(self.sfp_index, config, Common.NULL_VAL)

        presence_status = False if output == '1' else True
    ...
    def get_transceiver_info(self):
        ...
        transceiver_info_dict = dict.fromkeys(self.info_dict_keys, Common.NULL_VAL)
        if not self.get_presence():
            return transceiver_info_dict

        transceiver_info_dict['type'] = self._type

        if self._type == self.QSFP_TYPE:
            offset = 128
        ...
        if self._type == self.QSFP_TYPE:
            for key in self.qsfp_cable_length_tup:
    ...
    def get_transceiver_bulk_status(self):
        ...
        ========================================================================
        """
        transceiver_dom_info_dict = dict.fromkeys(self.dom_dict_keys, Common.NULL_VAL)
    ...
    def get_transceiver_threshold_info(self):
        ...
        transceiver_dom_threshold_info_dict = dict.fromkeys(
            self.threshold_dict_keys, Common.NULL_VAL)
    ...
    def get_lpmode(self):
        ...
        lpmode_status = True if output == '1' else False

        return lpmode_status
    ...
    def reset(self):
        ...
        output1 = self._api_common.set_output(self.sfp_index, "0", config)
        
        # Sleep 1 second to allow it to settle
        time.sleep(1)
        # Flip the bit back high and write back to the register to take port out of reset
        output2 = self._api_common.set_output(self.sfp_index, "1", config)
    ...
    def set_lpmode(self, lpmode):
        ...
        config = self._config.get(f_name)
        if type(lpmode) == bool:
            lpmode = "True" if lpmode == True else "False"

        output = self._api_common.set_output(self.sfp_index, lpmode, config)

    ...
    def getETValue(a, filename, tagname):
        ...
        for neighbor in root.iter(tagname):
            ...
            for pros in neighbor.iter("property"):
                ...
                if ret.get('e2type') == 'tlv' and ret.get("name") == "tlv":
                    tlvval = checktype.getfantlvValue(prob_t, root, ret["location"])
                    if  isinstance(tlvval, str) and tlvval.startswith("ERR"):
                        prob_t['errcode']= -1
                        prob_t['errmsg']= tlvval
                        break
                    prob_t.update(tlvval)
                    continue

...
#############################################################################
# Asterfusion CX312P-48Y-T SFP Plugin
#
...
class SfpUtil(SfpUtilBase):
    ...
    def get_presence(self, port_num):
        ...
        with ThriftConnect() as thrift_client:
            if port_num < self.QSFP_PORT_START:
                presence = thrift_client.pltfm_mgr_sfp_presence_get(port_num + 1)
            elif port_num >= self.QSFP_PORT_START and port_num < self.QSFP_PORT_END:
                presence = thrift_client.pltfm_mgr_qsfp_presence_get(port_num + 1 - self.QSFP_PORT_START)
            elif port_num >= self.QSFP_PORT_END:
                #index 60 61 map to 49 50
                presence = thrift_client.pltfm_mgr_sfp_presence_get(port_num + 1 - 12)
        return presence
    ...
    def get_eeprom(self, port_num):
        ...
        with ThriftConnect() as thrift_client:
            if port_num < self.QSFP_PORT_START:
                eeprom_hex = thrift_client.pltfm_mgr_sfp_info_get(port_num + 1)
            elif port_num >= self.QSFP_PORT_START and port_num < self.QSFP_PORT_END:
                eeprom_hex = thrift_client.pltfm_mgr_qsfp_info_get(port_num + 1 - self.QSFP_PORT_START)
            elif port_num >= self.QSFP_PORT_END:
                #index 60 61 map to 49 50
                eeprom_hex = thrift_client.pltfm_mgr_sfp_info_get(port_num + 1 - 12)

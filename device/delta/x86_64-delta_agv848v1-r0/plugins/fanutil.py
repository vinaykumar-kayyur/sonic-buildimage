...
class FanUtil(FanBase):
    ...
    EEPROM_BASE_BUS_NUM = 21
    FAN_NUM = 4
    GET_HWSKU_CMD = "sonic-cfggen -d -v DEVICE_METADATA.localhost.hwsku"
    sku_without_fan_direction = ['Delta-agv848v1']
    ...
    def get_speed(self, index):
        #fan{}_input:2~5
        num_fan=index+1
        speed = self._read_file(self.fan_path,self.fan_get_speed, num_fan)
        return speed

    def get_speed_rear(self, index):
        #fan{}_input:2~5
        num_fan=index+1
        speed = self._read_file(self.fan_path_rear,self.fan_get_speed, num_fan)
        return speed
    ...
    def set_speed(self, val):
        ...
        for index in range(1,self.num_of_fan+1):
            #fan{}_input_percentage:2~5
            num_fan=index+1
            try:
                with open(os.path.join(self.fan_path, self.fan_set_speed.format(num_fan)), 'w') as fan_percentage:
                    fan_percentage.write(str(val))
            except (ValueError, IOError):
                log_err("Read file {} failed".format(self.fan_path + self.fan_set_speed.format(num_fan)))
                status = False

            try:
                with open(os.path.join(self.fan_path_rear, self.fan_set_speed.format(num_fan)), 'w') as fan_percentage:
                    fan_percentage.write(str(val))
            except (ValueError, IOError):
                log_err("Read file {} failed".format(self.fan_path + self.fan_set_speed.format(num_fan)))
                status = False

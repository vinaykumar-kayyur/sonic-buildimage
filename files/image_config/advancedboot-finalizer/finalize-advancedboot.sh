...
# Define components that needs to reconcile during advanced
# boot:
...
function debug()
{
    /usr/bin/logger "ADVANCED_BOOT_FINALIZER : $1"
...
{
    ADVANCED_BOOT=`sonic-db-cli STATE_DB hget "ADVANCED_RESTART_ENABLE_TABLE|system" enable`
}
...
{
    sonic-db-cli STATE_DB hget "ADVANCED_RESTART_TABLE|$1" state
}
...
{
    debug "Restoring counters folder after advanced boot..."
...
if [[ x"${ADVANCED_BOOT}" != x"true" ]]; then
    debug "advanced boot is not enabled ..."
    exit 0
fi
...
debug "Save in-memory database after advanced reboot ..."
config save -y

    ...
    if len(downloaded_cert_names):
        for cert_name in downloaded_cert_names:
            ...
            if cert_name not in existing_cert_names:
                logger.log_info("cert_converter : convert_certs : Start converting "+cert_name)
                ...
                logger.log_info("cert_converter : convert_certs : "+cmd)
                if not execute_cmd(cmd):
                    logger.log_error("cert_converter : convert_certs : Extracting crt from pfx failed!", True)
                    return False                
                ...
                if not execute_cmd(cmd):
                    logger.log_error("cert_converter : convert_certs : Creating private key from pfx failed!", True)
                    return False
                # Decrypt the private key
                cmd = "openssl rsa -in "+certs_path+"private.key -out "+certs_path+name+".key."+ver+" -passin pass:"+random_password
                if not execute_cmd(cmd):
                    logger.log_error("cert_converter : convert_certs : Extracting key from pfx failed!", True)
                    return False
...
def main():
    while True:
        logger.log_info("cert_converter : main : Check if uber_notify_file is present")
        if os.path.isfile(uber_notify_file_path):
            logger.log_info("cert_converter : main : uber_notify_file found, converting all certs...")
    ...
    while True:
        logger.log_notice("cert_converter : main : Checking for cert changes...")
        if not convert_certs(acms_certs_path, certs_path, password_length):
            logger.log_error("cert_converter : main : Cert conversion failed!")

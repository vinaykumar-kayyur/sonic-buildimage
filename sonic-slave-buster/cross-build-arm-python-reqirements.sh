...
do
    ...
    mkdir -p wheel-download/sources && pip download --platform $PY_PLAT --no-deps --dest wheel-download $pkg
    count=`ls -1 wheel-download/*.gz  2>/dev/null | wc -l`
    if [ $count != 0 ]
    then 
        tar xvf  wheel-download/*.gz -C wheel-download/sources

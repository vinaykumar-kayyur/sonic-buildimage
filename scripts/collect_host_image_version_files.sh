...
mkdir -p target/vcache/${HOST_IMAGE_NAME}
...
sudo chmod -f 777 ${LOCAL_CACHE_DIR} 

if [ ! -z ${SONIC_VERSION_CACHE} ]; then

	#cat ${LOCAL_CACHE_DIR}/cache.base64| base64 -d  >${LOCAL_CACHE_FILE}
	#rm -f ${LOCAL_CACHE_DIR}/cache.base64
	...
	VERSION_FILES="${HOST_IMAGE_SRC_PATH}/versions-*-${DISTRO}-${ARCH} ${HOST_IMAGE_SRC_PATH}/versions-*"
	...
	sudo chmod -f 777 ${GLOBAL_CACHE_DIR}
	BIN_CACHE_PATH=${FILESYSTEM_ROOT}/sonic/target/vcache/${HOST_IMAGE_NAME}
	BIN_CACHE_FILE=${BIN_CACHE_PATH}/cache.tgz
	cp ${BIN_CACHE_FILE} ${LOCAL_CACHE_FILE}

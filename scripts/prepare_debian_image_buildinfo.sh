...
if [ ! -z ${SONIC_VERSION_CACHE} ]; then
	...
	VERSION_FILES="${HOST_IMAGE_PATH}/versions-*-${DISTRO}-${ARCH} ${HOST_IMAGE_PATH}/versions-*"
	...
	if [[ -e ${GLOBAL_CACHE_FILE} ]]; then
		...
		for VERSION in ${VERSIONS[@]}; do
			VERSION_PREV_SHA="$( git --no-pager show $(ls -f ${DEP_FILES} ${VERSION_FILES}|sed 's|.*|'${VERSION}':&|g') | sha1sum | awk '{print substr($1,0,23);}')"
			GLOBAL_PREV_CACHE_FILE=${GLOBAL_CACHE_DIR}/${HOST_IMAGE_NAME}-${VERSION_PREV_SHA}.tgz
	...
	cp ${LOCAL_CACHE_FILE} ${BIN_CACHE_FILE}

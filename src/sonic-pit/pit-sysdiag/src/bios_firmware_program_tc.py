...
MASTER_ORIG_FW_CFG = "bios_master_firmware_baseline.json"
SLAVE_ORIG_FW_CFG = "bios_slave_firmware_baseline.json"


class BIOSFIRMWAREPROGRAMTC(TestCaseCommon):
    ...
    def __init__(self, index, logger, platform_cfg_file, case_cfg_file=None):
        ...
        MODULE_NAME = "bios_firmware_program_tc"
        ...
        try:
            self.firmware_type = self.platform_cfg_json["bios_firmware_type"]
            self.test_firmware_list = self.platform_cfg_json["bios_firmware_test"]
            self.baseline_firmware_list = self.platform_cfg_json["bios_firmware_baseline"]
    ...
    def get_upgrade_path(self, firmware_list, item):
        ...
        if "absolute_path" not in firmware_list[item]:
            ...
            # True if absolute_path does not set
        if skip_upgrade is True:
            ...
            # Check whether slave file exist
            if (
            ...
            # Get secondary flash firmware file path, @same_with_master defualt False,
            # use firmware_list[item]["file_path"] if secondary flash firmware file path
            # is the same with primary flash, else use firmware_list[item]["slave_file_path"]
        else:
            if (
        ...
        if self.slave_upgrade:
            if (
                ...
                if (
                    ...
                    # if "slave_upgrade" not set, False by default
                    return False, True
                else:
                    raise Exception(
                    ...
                    # if "slave_upgrade" != baseline fw, throw exception, JSON file invalid
            elif (
                self.test_firmware_list[item]["slave_upgrade"] is False
                and self.baseline_firmware_list[item]["slave_upgrade"] is False
            ):
                return False, True
            # "slave_upgrade" is False, does not support upgrade secondary flash
            elif (
            ...
            # test fw "slave_upgrade" != baseline fw, throw exception, JSON file invalid
        ...
        try:
            for item in self.firmware_type:
                ...
                if skip is True:
                    ...
                    continue
                # Skip if slave_upgrade not exist

...
class SysDiag(object):
    def __init__(self, logger):
        self.log_json_file_dir = "/var/log/"
    ...
    def generate_test_case_list(self):
        ...
        self.log.log_info("Generate test case list ...", self.console_print_flag)
        index = 1
    ...
    def run_one_case(self, tc):
        ...
        try:
            self.log.log_info(
                ...
                self.console_print_flag)
            tc.get_start_time()
            ret = tc.run_test(self.case_test_count)
            tc.get_stop_time()
    ...
    def print_final_result_to_json(self):
        ...
        info['detailed_result']['pass_case'] = list(self.pass_case_dict.keys())
        info['detailed_result']['failed_case'] = list(self.fail_case_dict.keys())
        result["result"] = info
        result["details"] = self.cases_result
        search_cmd = "ls {} | grep sysdiag_result | wc -l".format(
            self.log_json_file_dir)
        search_status, search_log = run_command(search_cmd)
        if search_status != 0 or int(search_log) < 0:
            self.log.log_info("find filename error!", self.console_print_flag)
            log_name = self.log_json_file_dir + "sysdiag_result.json"
        else:
            if int(search_log) == 0:
                log_name = self.log_json_file_dir + "sysdiag_result.json"
            else:
                log_name = self.log_json_file_dir + \
                    "sysdiag_result{}.json".format(int(search_log))
        with open(log_name, "w") as f:
            json.dump(result, f, indent=4, separators=(',', ': '))
        json_list = []
        for files in os.listdir(self.log_json_file_dir):
        ...
        if int(search_log) >= 1:
            ...
            run_command(
                "mv {} {}".format(
                    "{}sysdiag_result.json".format(
                        self.log_json_file_dir), "{}sysdiag_result1.json".format(
                            self.log_json_file_dir)))
            oldfile = os.path.join(
                self.log_json_file_dir,
                "sysdiag_result{}.json".format(
                    len(log_list)))
            newfile = os.path.join(self.log_json_file_dir, "sysdiag_result.json")
            os.rename(oldfile, newfile)

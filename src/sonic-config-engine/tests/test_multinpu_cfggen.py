...
class TestMultiNpuCfgGen(TestCase):
    ...
    def test_metadata_ntp(self):
        argument = '-m "' + self.sample_graph  + '" -v "NTP_SERVER"'
        output = self.run_script(argument)
        self.assertEqual(output.strip(), "{'17.39.1.130': {}, '17.39.1.129': {}}")
    ...
    def test_mgmt_port(self):
        argument = '-m "' + self.sample_graph +  '" -v "MGMT_PORT"'
        output = self.run_script(argument)
        self.assertEqual(output.strip(), "{'eth0': {'alias': 'eth0', 'admin_status': 'up'}}")
        for asic in range(NUM_ASIC):
            output = self.run_script_for_asic(argument, asic, self.port_config[asic])
            self.assertEqual(output.strip(), "{}")
    
    def test_frontend_asic_portchannels(self):
        argument = "-m {} -p {} -n asic0 -v \"PORTCHANNEL\"".format(self.sample_graph, self.port_config[0])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
    ...
    def test_backend_asic_portchannels(self):
        argument = "-m {} -p {} -n asic3 -v \"PORTCHANNEL\"".format(self.sample_graph, self.port_config[3])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
    ...
    def test_frontend_asic_portchannel_mem(self):
        argument = "-m {} -p {} -n asic0 -v \"PORTCHANNEL_MEMBER.keys()\"".format(self.sample_graph, self.port_config[0])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
            "[('PortChannel4002', 'Ethernet-BP8'), ('PortChannel0002', 'Ethernet0'), ('PortChannel4001', 'Ethernet-BP4'), ('PortChannel4001', 'Ethernet-BP0'), ('PortChannel0002', 'Ethernet4'), ('PortChannel4002', 'Ethernet-BP12')]")
    
    def test_backend_asic_portchannels_mem(self):
        argument = "-m {} -p {} -n asic3 -v \"PORTCHANNEL_MEMBER.keys()\"".format(self.sample_graph, self.port_config[3])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
            "[('PortChannel4013', 'Ethernet-BP388'), ('PortChannel4014', 'Ethernet-BP392'), ('PortChannel4014', 'Ethernet-BP396'), ('PortChannel4013', 'Ethernet-BP384')]")

    def test_frontend_asic_portchannel_intf(self):
        argument = "-m {} -p {} -n asic0 -v \"PORTCHANNEL_INTERFACE.keys()\"".format(self.sample_graph, self.port_config[0])        
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
            "[('PortChannel4002', '10.1.0.3/31'), 'PortChannel0002', ('PortChannel0002', 'FC00::1/126'), 'PortChannel4001', ('PortChannel0002', '10.0.0.0/31'), ('PortChannel4001', '10.1.0.1/31'), 'PortChannel4002']")
    
    def test_backend_asic_portchannel_intf(self):
        argument = "-m {} -p {} -n asic3 -v \"PORTCHANNEL_INTERFACE.keys()\"".format(self.sample_graph, self.port_config[3])        
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
            "['PortChannel4013', ('PortChannel4014', '10.1.0.6/31'), ('PortChannel4013', '10.1.0.2/31'), 'PortChannel4014']")
    
    def test_frontend_asic_device_neigh(self):
        argument = "-m {} -p {} -n asic0 -v \"DEVICE_NEIGHBOR\"".format(self.sample_graph, self.port_config[0])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
    ...
    def test_frontend_asic_device_neigh_metadata(self):
        argument = "-m {} -p {} -n asic0 -v \"DEVICE_NEIGHBOR_METADATA\"".format(self.sample_graph, self.port_config[0])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
    ...
    def test_backend_asic_device_neigh(self):
        argument = "-m {} -p {} -n asic3 -v \"DEVICE_NEIGHBOR\"".format(self.sample_graph, self.port_config[3])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
    ...
    def test_backend_device_neigh_metadata(self):
        argument = "-m {} -p {} -n asic3 -v \"DEVICE_NEIGHBOR_METADATA\"".format(self.sample_graph, self.port_config[3])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
    ...
    def test_frontend_bgp_neighbor(self):
        argument = "-m {} -p {} -n asic0 -v \"BGP_NEIGHBOR\"".format(self.sample_graph, self.port_config[0])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \
    ...
    def test_backend_asic_bgp_neighbor(self):
        argument = "-m {} -p {} -n asic3 -v \"BGP_NEIGHBOR\"".format(self.sample_graph, self.port_config[3])
        output = self.run_script(argument)
        self.assertEqual(output.strip(), \

...
class TestJ2Files(TestCase):
    ...
    def test_dhcp_relay(self):
        ...
        # Test generation of docker-dhcp-relay.supervisord.conf of minigraph with secondary subnets
        template_path = os.path.join(self.test_dir, '..', '..', '..', 'dockers', 'docker-dhcp-relay', 'docker-dhcp-relay.supervisord.conf.j2')
        argument = ['-m', self.t0_minigraph_secondary_subnets, '-p', self.t0_port_config, '-t', template_path]
        self.run_script(argument, output_file=self.output_file)
        self.assertTrue(utils.cmp(os.path.join(self.test_dir, 'sample_output', utils.PYvX_DIR, 'docker-dhcp-relay-secondary-subnets.supervisord.conf'), self.output_file))

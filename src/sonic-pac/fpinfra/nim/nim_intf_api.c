  ...
  else
  {
    ...
    if (rc ==  SUCCESS)
    {
        ...
        if (strlen(( uchar8*)ifAlias) <=  NIM_IF_ALIAS_SIZE)
          strcpy( nimCtlBlk_g->nimPorts[intIfNum].configPort.cfgInfo.ifAlias, ( uchar8*)ifAlias );
        else
          strncpy( nimCtlBlk_g->nimPorts[intIfNum].configPort.cfgInfo.ifAlias, ( uchar8*)ifAlias, ( NIM_IF_ALIAS_SIZE) );

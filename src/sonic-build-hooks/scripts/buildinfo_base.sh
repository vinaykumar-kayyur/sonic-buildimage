...
run_python_command()
{
	...
	if [[  -e .eggs || $1 != "setup.py" || ( $2 != "bdist_wheel" && $2 != "test" ) \
		|| "${SKIP_BUILD_HOOK}" == Y || "${ENABLE_VERSION_CONTROL_PY}" != "y" ]]; then
		$REAL_COMMAND "$@"
		local result=$?
		return ${result}
	fi
	...
	if [[ ! -e ${PYTHON_CACHE_PATH} ]]; then
		${SUDO} mkdir -p ${PYTHON_CACHE_PATH}
		chmod 777 ${PYTHON_CACHE_PATH}
	fi

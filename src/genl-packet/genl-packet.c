 ...
 * Modified to support generic packet ins with custom metadata.
 *
                             ...
                             unsigned int context)
{
  ...
  /* Metalength is sum of netlink message sizes of in_ifindex + out_ifindex +
    * context. Be sure to add any extension fields here. */
  BUILD_BUG_ON(GENL_PACKET_ATTR_MAX != 3);
  meta_len = nla_total_size(sizeof(s16)) +
             nla_total_size(sizeof(s16)) +
             nla_total_size(sizeof(u32));

  // Shrink the packet data to fit in the genl message.
  data_len = skb->len;
  if (meta_len + nla_total_size(data_len) > GENL_PACKET_MAX_PACKET_SIZE)
    data_len =
      GENL_PACKET_MAX_PACKET_SIZE - meta_len - NLA_HDRLEN - NLA_ALIGNTO;
  if (data_len <= 0) {
    pr_alert_once("genl_packet metadata exceeds maximum message size\n");
    return;
  }
  ...
  if (unlikely(!nl_skb)) {
      pr_crit_ratelimited(
          "Failed to allocate netlink message for genl_packet\n");
      return;
  }

  // Add the metadata and packet payload to the netlink message
...
MODULE_AUTHOR("PINS Working Group");
MODULE_DESCRIPTION("genetlink channel for genl_packet");

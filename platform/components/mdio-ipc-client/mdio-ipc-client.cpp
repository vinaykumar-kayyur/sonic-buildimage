...
extern "C" sai_status_t mdio_write(uint64_t platform_context, uint32_t mdio_addr,
...
extern "C" sai_status_t mdio_write_cl22(uint64_t platform_context, uint32_t mdio_addr,
        uint32_t reg_addr, uint32_t number_of_registers, const uint32_t *data)
{
    int rc = SAI_STATUS_FAILURE;
    char cmd[SAI_IPC_BUFF_SIZE], resp[SAI_IPC_BUFF_SIZE];

 ...
 *      -- In make menuconfig, select CUST3 PHY: Chip Support ---> [*] CUSTOMER PHY 3
 *         Macro "CONFIG_SDK_PHY_CUST3" will be enabled.
 *      -- The SDK defined PHY type is RTK_PHYTYPE_CUST3.
 *      -- If vender PHY C code needed to be build through SDK make, the following makefile can be used:
 *          sdk/build/Makefile.custPhy3
 *      -- Function: phy_identify_cust3() is used for checking if the port is CUST3 PHY in "PHY probe" stage during system init
 *          You need to know the PHY ID of the PHY for implement this function.
 *      -- Function: phy_cust3drv_mapperInit() is the initial for registering driver's callbacks of the PHY
 ...
 *          phy_cust3_sampleCodePortingMdio_read()
 *          phy_cust3_sampleCodePortingMdio_write()
 */
...
#define PHY_CUST3_PORT_NUM                  1

/* !! PHY ID of the CUST3 PHY */
#define PHY_CUST3_PHYID_REG2                0x5555
#define PHY_CUST3_PHYID_REG3                0xEEEE

/*
 * Data Declaration
 */
rt_phyInfo_t phy_cust3_info =
{
    .phy_num    = PHY_CUST3_PORT_NUM,
 ...
 *      phy_identify_cust3
 * Description:
 *      Identify the port is CUST3 PHY or not
 *      This function will be called in "phy probe" stage for checking if the port's PHY is CUST3 PHY
 * Input:
 *      unit     - unit id
 *      port     - port id
 ...
 *      None
 * Return:
 *      RT_ERR_OK               - port is CUST3 PHY, the port will then hook with this PHY driver
 *      RT_ERR_FAILED           - access failure or others
 *      RT_ERR_PHY_NOT_MATCH    - port is not CUST3 PHY
 * Note:
 *      None
 */
int32
phy_identify_cust3(uint32 unit, rtk_port_t port, uint32 model_id, uint32 rev_id)
{
    uint32          reg1d2 = 0, reg1d3 = 0;
    hal_control_t   *pHalCtrl = NULL;
    drv_smi_mdxProtoSel_t   mdxProto;

    RT_LOG(LOG_INFO, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 identify", unit, port);
    ...
    /* !! Sample code of read PMA/PMD device identifier (Registers 1.2, 1.3) and check if there is a match of CUST3 PHY */
    ...
    if ((reg1d2 == PHY_CUST3_PHYID_REG2) && (reg1d3 == PHY_CUST3_PHYID_REG3))
    {
        goto EXIT_OK;
    }

    /* recover SMI bus setting for this bus is not attached to CUST3 PHY */
    if (mdxProto != DRV_SMI_MDX_PROTO_C45)
    {
  ...
  EXIT_OK:
    ...
    RT_LOG(LOG_INFO, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 identify match", unit, port);
    return RT_ERR_OK;
}
 ...
 */
int32
phy_cust3_media_get(uint32 unit, rtk_port_t port, rtk_port_media_t *pMedia)
{
 ...
 */
int32
phy_cust3_autoNegoEnable_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
 ...
 */
int32
phy_cust3_autoNegoEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    RT_LOG(LOG_INFO, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 autoNegoEnable_set:%u", unit, port, enable);
 ...
 */
int32
phy_cust3_autoNegoAbility_get(uint32 unit, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    /* !! call the vender phy driver to set autonegotiation enable status */
 ...
 */
int32
phy_cust3_autoNegoAbility_set(uint32 unit, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{

    RT_LOG(LOG_INFO, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 autoNegoAbility_set", unit, port);
 ...
 */
int32
phy_cust3_duplex_get(uint32 unit, rtk_port_t port, uint32 *pDuplex)
{
 ...
 */
int32
phy_cust3_duplex_set(uint32 unit, rtk_port_t port, uint32 duplex)
{
    RT_LOG(LOG_INFO, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 duplex_set:%u", unit, port, duplex);
 ...
 */
int32
phy_cust3_speed_get(uint32 unit, rtk_port_t port, uint32 *pSpeed)
{
 ...
 */
int32
phy_cust3_speed_set(uint32 unit, rtk_port_t port, uint32 speed)
{

    RT_LOG(LOG_INFO, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 speed_set:%u", unit, port, speed);
 ...
 */
int32
phy_cust3_enable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
 ...
 */
int32
phy_cust3_eeeEnable_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
 ...
 */
int32
phy_cust3_eeeEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
 ...
 */
int32
phy_cust3_crossOverMode_get(uint32 unit, rtk_port_t port, rtk_port_crossOver_mode_t *pMode)
{
 ...
 */
int32
phy_cust3_crossOverMode_set(uint32 unit, rtk_port_t port, rtk_port_crossOver_mode_t mode)
{
 ...
 */
int32
phy_cust3_crossOverStatus_get(uint32 unit, rtk_port_t port, rtk_port_crossOver_status_t *pStatus)
{
 ...
 */
int32
phy_cust3_loopback_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
 ...
 */
int32
phy_cust3_loopback_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
 ...
 */
int32
phy_cust3_rtctResult_get(uint32 unit, rtk_port_t port, rtk_rtctResult_t *pRtctResult)
{
 ...
 */
int32
phy_cust3_rtct_start(uint32 unit, rtk_port_t port)
{
 ...
 */
int32
phy_cust3_ieeeTestMode_set(uint32 unit, rtk_port_t port, rtk_port_phyTestMode_t *pTestMode)
{
 ...
 */
int32
phy_cust3_reg_mmd_get(uint32 unit, rtk_port_t port, uint32 mmdAddr, uint32 mmdReg, uint32 *pData)
{
    int32       ret;

    if ((ret = hal_miim_mmd_read(unit, port, mmdAddr, mmdReg, pData)) != RT_ERR_OK)
    {
        RT_LOG(LOG_WARNING, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 mmd 0x%X reg 0x%X get fail %x", unit, port, mmdAddr, mmdReg, ret);
 ...
 */
int32
phy_cust3_reg_mmd_set(uint32 unit, rtk_port_t port, uint32 mmdAddr, uint32 mmdReg, uint32 data)
{
    int32       ret;

    if ((ret = hal_miim_mmd_write(unit, port, mmdAddr, mmdReg, data)) != RT_ERR_OK)
    {
        RT_LOG(LOG_WARNING, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 mmd 0x%X reg 0x%X set fail %x", unit, port, mmdAddr, mmdReg, ret);
        return RT_ERR_FAILED;
    }
 ...
 */
int32
phy_cust3_init(uint32 unit, rtk_port_t port)
{

    RT_LOG(LOG_INFO, (MOD_HAL|MOD_PHY), "unit %u port %u cust3 init", unit, port);
 ...
 */
void
phy_cust3drv_mapperInit(rt_phydrv_t *pPhydrv)
{
     ...
     */
    pPhydrv->phydrv_index = RT_PHYDRV_CUST3;
    pPhydrv->fPhydrv_init = phy_cust3_init;
    pPhydrv->fPhydrv_media_get = phy_cust3_media_get;
    pPhydrv->fPhydrv_autoNegoEnable_get = phy_cust3_autoNegoEnable_get;
    pPhydrv->fPhydrv_autoNegoEnable_set = phy_cust3_autoNegoEnable_set;
    pPhydrv->fPhydrv_autoNegoAbility_get = phy_cust3_autoNegoAbility_get;
    pPhydrv->fPhydrv_autoNegoAbility_set = phy_cust3_autoNegoAbility_set;
    pPhydrv->fPhydrv_duplex_get = phy_cust3_duplex_get;
    pPhydrv->fPhydrv_duplex_set = phy_cust3_duplex_set;
    pPhydrv->fPhydrv_speed_get = phy_cust3_speed_get;
    pPhydrv->fPhydrv_speed_set = phy_cust3_speed_set;
    pPhydrv->fPhydrv_enable_set = phy_cust3_enable_set;

    pPhydrv->fPhydrv_reg_mmd_get = phy_cust3_reg_mmd_get;
    pPhydrv->fPhydrv_reg_mmd_set = phy_cust3_reg_mmd_set;

#if !defined(__BOOTLOADER__)
    pPhydrv->fPhydrv_eeeEnable_get = phy_cust3_eeeEnable_get;
    pPhydrv->fPhydrv_eeeEnable_set = phy_cust3_eeeEnable_set;
    pPhydrv->fPhydrv_crossOverMode_get = phy_cust3_crossOverMode_get;
    pPhydrv->fPhydrv_crossOverMode_set = phy_cust3_crossOverMode_set;
    pPhydrv->fPhydrv_crossOverStatus_get = phy_cust3_crossOverStatus_get;
    pPhydrv->fPhydrv_masterSlave_get = phy_cust3_masterSlave_get;
    pPhydrv->fPhydrv_masterSlave_set = phy_cust3_masterSlave_set;
    pPhydrv->fPhydrv_loopback_get = phy_cust3_loopback_get;
    pPhydrv->fPhydrv_loopback_set = phy_cust3_loopback_set;
    pPhydrv->fPhydrv_ieeeTestMode_set = phy_cust3_ieeeTestMode_set;
    pPhydrv->fPhydrv_rtctResult_get = phy_cust3_rtctResult_get;
    pPhydrv->fPhydrv_rtct_start =  phy_cust3_rtct_start;
#endif
 ...
 */
void
phy_cust3_sampleCodePortingMdio_write (unsigned int unit, unsigned int port, unsigned int mmd, unsigned int address, unsigned int data)
{
    int32           ret;
 ...
 */
unsigned int
phy_cust3_sampleCodePortingMdio_read (unsigned int unit, unsigned int port, unsigned int mmd, unsigned int address)
{
    uint32          data = 0;
    int32           ret;

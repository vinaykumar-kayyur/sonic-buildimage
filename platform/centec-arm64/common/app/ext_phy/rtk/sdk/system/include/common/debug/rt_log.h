...
extern FILE *cust_rtk_log_f;
extern void syslog(int priority, const char *message, ...);

#define STR_CUST_RT_LOG_NOTICE      "NOTICE"
#define STR_CUST_RT_LOG_NORMAL      "NORMAL"
#define STR_CUST_RT_LOG_WARN        "WARN"
#define STR_CUST_RT_LOG_ERR         "ERR"
#define LOG_LEVEL_STR(V)            STR_##V

#define CUST_RTK_LOG_FILE_DIR        "/var/log/rtk.log"
#define CUST_RTK_LOG(level, fmt, args...) \
    do { \
        time_t timetemp; \
        struct tm *p; \
        time(&timetemp); \
        ...
        if (!cust_rtk_log_f) { \
            printf("rtk_log_file null, msg:[%s]%s:%d[%d:%d:%d] "fmt, LOG_LEVEL_STR(level), __FUNCTION__, __LINE__, p->tm_hour, p->tm_min, p->tm_sec,  ##args); \
	    break;\
        } \
        fprintf(cust_rtk_log_f, "[%s]%s:%d[%d:%d:%d] "fmt, LOG_LEVEL_STR(level), __FUNCTION__, __LINE__, p->tm_hour, p->tm_min, p->tm_sec,  ##args); \
        fflush(cust_rtk_log_f); \
    } while (0)

...
class AVSUTIL:
    ...
    def mac_avs_chip(bus, devno, loc, open, close, loop, protectaddr, level, loopaddr):
        # disable protection
        wbi2cset(bus, devno, protectaddr, open)
        wbi2cset(bus, devno, loopaddr, loop)
        wbi2cset_word(bus, devno, loc, level)
        ret, value = wbi2cget_word(bus, devno, loc)
        if strtoint(value) == level:
            ret = 0
        # enable protection
        wbi2cset(bus, devno, protectaddr, close)
        if ret == 0:
            return True
        return False
...
class I2CUTIL:
    ...
    def writeToFanE2(bus, loc, rst_arr):
        index = 0
        for item in rst_arr:
            wbi2cset(bus, loc, index, ord(item))
            index += 1
...
def fac_init_cardidcheck():
    rest = getsyseeprombyId(TLV_CODE_CARID)  # check cardId same or not
    if rest is None:
...
def wbpcird(pcibus, slot, fn, bar, offset):
    """read pci register"""
    if offset % 4 != 0:
        return False
...
def wbpciwr(pcibus, slot, fn, bar, offset, data):
    """write pci register"""
    ret = inttostr(data, 4)
    filename = "/sys/bus/pci/devices/0000:%02x:%02x.%x/resource%d" % (int(pcibus), int(slot), int(fn), int(bar))
    with open(filename, "r+") as file:
...
def wbi2cget_word(bus, devno, address):
...
def wbi2cset_word(bus, devno, address, byte):
...
def checkfansninput(fan_sn, fansntemp):
    if fan_sn in fansntemp:
        PRINTERR("exist same Serial Number, please input again")
        return False
    if len(fan_sn) != 13:
        PRINTERR("Serial Number length incorrect, please input again")
        return False
    return True
...
def checkfanhwinput(hw):
    if len(hw) != 4:
        PRINTERR("hardware version length incorrect, please input again")
        return False
    if hw.find(".") != 1:
        PRINTERR("hardware version incorrect, please input again")
        return False
    return True
...
def get_fane2_sysfs(bus, loc):
    wb_fan_e2 = "%d-%04x/fan" % (bus, loc)
    eeprom = get_sysfs_value(wb_fan_e2)
    return eeprom


def util_show_fane2():
    ret = sorted(I2CUTIL.getvaluefromdevice("wb_fan"))
    if len(ret) <= 0:
        return None
    fans = []
...
def fac_fans_setmac():
    ret = I2CUTIL.getvaluefromdevice("wb_fan")
...
def writeToEEprom(rst_arr):
    ...
    index = 0
    for item in rst_arr:
        wbi2cset(E2_LOC["bus"], E2_LOC["devno"], index, ord(item))
        index += 1
...
def get_sys_eeprom():
    eeprom = get_sysfs_value(wb_eeprom)

...
static int pega_porsche_psu_read(unsigned short addr, u8 reg)
{
             ...
             char *buf)
{
    ...
    data = pega_porsche_psu_read(client->addr, reg);
...
static struct attribute *pega_porsche_psu_attributes[] = {
...
static const struct attribute_group pega_porsche_psu_group = { .attrs = pega_porsche_psu_attributes};

static void pega_porsche_psu_add_client(struct i2c_client *client)
{
...
static void pega_porsche_psu_remove_client(struct i2c_client *client)
{
    struct list_head        *list_node = NULL;
    struct psu_client_node *psu_node = NULL;
    int found = 0;
...
static int pega_porsche_psu_probe(struct i2c_client *client,
...
{  
    ...
    switch(client->addr)
    {
        case PSU_58_ADDRESS:
        case PSU_59_ADDRESS:
            status = sysfs_create_group(&client->dev.kobj, &pega_porsche_psu_group);
            break;
        default:
    ...
    pega_porsche_psu_add_client(client);
...
static int pega_porsche_psu_remove(struct i2c_client *client)
{
    switch(client->addr)
    {
        case PSU_58_ADDRESS:
        case PSU_59_ADDRESS:
            sysfs_remove_group(&client->dev.kobj, &pega_porsche_psu_group);
            break;
        default:
    ...
    pega_porsche_psu_remove_client(client);  
    return 0;
}

static const struct i2c_device_id pega_porsche_psu_id[] = {
    { "porsche_psu", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, pega_porsche_psu_id);

static struct i2c_driver pega_porsche_psu_driver = {
    .class      = I2C_CLASS_HWMON,
    .driver = {
        .name = "pegatron_porsche_psu",
    },
    .probe      = pega_porsche_psu_probe,
    .remove     = pega_porsche_psu_remove,
    .id_table   = pega_porsche_psu_id,
    .address_list = normal_i2c,
};

static int __init pega_porsche_psu_init(void)
{
    mutex_init(&list_lock);

    return i2c_add_driver(&pega_porsche_psu_driver);
}

static void __exit pega_porsche_psu_exit(void)
{
    i2c_del_driver(&pega_porsche_psu_driver);
}

MODULE_AUTHOR("Peter5 Lin <Peter5_Lin@pegatroncorp.com.tw>");
MODULE_DESCRIPTION("pega_porsche_psu driver");
MODULE_LICENSE("GPL");

module_init(pega_porsche_psu_init);
module_exit(pega_porsche_psu_exit);

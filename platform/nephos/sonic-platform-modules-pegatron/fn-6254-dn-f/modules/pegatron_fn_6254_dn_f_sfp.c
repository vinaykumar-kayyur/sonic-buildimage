...
static ssize_t fn_6254_dn_f_sfp_eeprom_read(struct fn_6254_dn_f_sfp_data *data, char *buf,
...
static ssize_t fn_6254_dn_f_sfp_read(struct fn_6254_dn_f_sfp_data *data,
		char *buf, loff_t off, size_t count)
{
	...
	while (count) {
		...
		status = fn_6254_dn_f_sfp_eeprom_read(data, buf, off, count);
		if (status <= 0) {
			if (retval == 0)
				retval = status;
			break;
		...
		char *buf, loff_t off, size_t count)
{
	int i;
	u8 cpldData = 0;
	struct fn_6254_dn_f_sfp_data *data;
...
check_done:
	...
	return fn_6254_dn_f_sfp_read(data, buf, off, count);
}
...
static int fn_6254_dn_f_sfp_device_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
	...
	data = kzalloc(sizeof(struct fn_6254_dn_f_sfp_data) , GFP_KERNEL);
	if (!data)
		return -ENOMEM;
...
MODULE_DESCRIPTION("fn_6254_dn_f_cpld_mux driver");
MODULE_LICENSE("GPL");

...
def get_fan_info():
    ...
    try:
        ...
        fan_num = 12 if hwver in (0x3080, 0x3081,) else \
                  10 if hwver in (0x3083,)         else 0
        fan_data = []
...
def get_temps_and_alarm():
    ...
    ghc1 = (int(data.tmp4) != -100)
    ghc2 = (int(data.tmp6) != -100)

    try:
        print("Far Left of Mainboard")
        print("Adapter: cgos adapter")
        print("M/B:          +%.1f C\n" % (float(data.tmp1)))

        print("Far Right of Mainboard")
        print("Adapter: cgos adapter")
        print("M/B:          +%.1f C\n" % (float(data.tmp2)))

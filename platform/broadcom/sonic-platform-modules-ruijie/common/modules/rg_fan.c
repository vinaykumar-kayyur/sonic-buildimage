...
static ssize_t show_fan_sysfs_tlv_value(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    memset(sysfs_buf, 0, 64);
    ret = platform_i2c_smbus_read_i2c_block_data(client, 0, sizeof(dfd_dev_head_info_t), (uint8_t *)&info);
    ...
    if (i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_READ_I2C_BLOCK)) {
        for (i = 0; i < info.tlv_len; i += 32)
            if (platform_i2c_smbus_read_i2c_block_data(client, sizeof(dfd_dev_head_info_t) + i, 32, tlv_data + i) != 32)
                break;
    }

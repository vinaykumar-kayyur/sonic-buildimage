...
typedef struct wb_lpc_device_s {
    u16 base;
    u16 size;
    u8  type;
    u8  id;
...
typedef enum wb_lpc_dev_type_s {
    LPC_DEVICE_CPLD         = 1,
    LPC_DEVICE_FPGA         = 2,
} wb_lpc_dev_type_t;

static wb_lpc_device_t g_wb_lpc_dev[] = {
...
static wb_lpc_device_t* lpc_get_device_info(int type, int id)
{
    int i;

    for (i = 0; i < ARRAY_SIZE(g_wb_lpc_dev); i++) {
        if ((g_wb_lpc_dev[i].type == type) && (g_wb_lpc_dev[i].id == id)) {
            return &g_wb_lpc_dev[i];
        }
    }
...
static int __init wb_lpc_cpld_init(void)
{
...
static void __exit wb_lpc_cpld_exit(void)
{
    struct pci_dev *pdev = NULL;
...
module_init(wb_lpc_cpld_init);
module_exit(wb_lpc_cpld_exit);
MODULE_LICENSE("GPL");

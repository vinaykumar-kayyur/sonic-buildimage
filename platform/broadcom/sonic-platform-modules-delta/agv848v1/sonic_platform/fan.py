...
##############################################
    ...
    def get_speed(self):
        ...
        if self.__is_psu_fan:
            ...
            if self.index == 1:
                speed = self._read_file(self.fan_path,self.fan_speed.format(self.fantrayindex+2))
            else:
                speed = self._read_file(self.fan_rpath,self.fan_speed.format(self.fantrayindex+2))
    ...
    def set_speed(self, speed):
        ...
        try:
            ...
            with open(os.path.join(path, self.fan_percentage_speed.format(self.fantrayindex+2)), 'w') as fan_percentage:
                                fan_percentage.write(str(speed))
        except (ValueError, IOError):
            log_err("Read file {} failed".format(self.fan_path + self.fan_percentage_speed.format(self.fantrayindex+2)))
            status = False

...
extern int cls_fpgapci_read(uint32_t fpga_read_addr);
extern int cls_fpgapci_write(uint32_t fpga_write_addr, uint32_t value);

int xcvr_fpgapci_read(uint32_t fpga_read_addr)
{
    return cls_fpgapci_read(fpga_read_addr);
}

int xcvr_fpgapci_write(uint32_t fpga_write_addr, uint32_t value)
{
	return cls_fpgapci_write(fpga_write_addr, value);
}
...
static ssize_t do_device_operation(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (strncmp(buf, "add", strlen(buf)-1)==0)
    {
        if (strcmp(cdata->dev_type, "pddf_xcvr")==0)
        {
            ...
            strcpy(board_info.type, cdata->dev_type); // nosemgrep
        ...
        else if((strcmp(cdata->dev_type, "optoe1")==0) || (strcmp(cdata->dev_type, "optoe2")==0)  || (strcmp(cdata->dev_type, "optoe3")==0))
        {
...
int __init pddf_data_init(void)
{
    ...
    ptr_fpgapci_read = xcvr_fpgapci_read;
    ptr_fpgapci_write = xcvr_fpgapci_write;

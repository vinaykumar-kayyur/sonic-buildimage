...
SENSOR_PARAM = {
    'Inlet U4 temp (EXHAUST)': [34, 47, None, None, NOMINAL_TEMP, FAN_DUTY_MIN, valid_if_exhaust],
    'Inlet U10 temp (EXHAUST)': [None, None, None, None, NOMINAL_TEMP, FAN_DUTY_MIN, valid_never],
    'Inlet U7 temp (INTAKE)': [34, 47, None, None, NOMINAL_TEMP, FAN_DUTY_MIN, valid_if_intake],
    'BCM inlet U60 temp': [54, 69, 105, 110, NOMINAL_TEMP, FAN_DUTY_MIN, valid_always],
    'CPU core temp': [69, 84, 91, 94, NOMINAL_TEMP, FAN_DUTY_MIN, valid_always]
}

class BelgiteFanControl(daemon_base.DaemonBase):
    global MODULE_NAME
    global SENSOR_PARAM
    ...
    def __init__(self, log_level):
        ...
        super(BelgiteFanControl, self).__init__(MODULE_NAME)
        ...
        try:
            ...
            self.log_error("Failed to init BelgiteFanControl due to {}, so exiting...".format(repr(e)))
            sys.exit(1)
    ...
    def thermal_shutdown(self, reason):
        cmd = ['/usr/local/bin/belgite_platform_shutdown.sh', reason]
...
def main(argv):
    ...
    fanctl = BelgiteFanControl(log_level)

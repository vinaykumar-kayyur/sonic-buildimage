/*
 * switchboard_fpga.c - driver for ds3000 Switch board FPGA/CPLD.
 *
...
#define CLASS_NAME "ds3000_fpga"
#define DRIVER_NAME "switchboard"
#define FPGA_PCI_NAME "ds3000_fpga_pci"
...
static struct platform_device *ds3000_dev;
...
struct ds3000_fpga_data *fpga_data;
...
static struct device * ds3000_sff_init(int portid) {
 ...
 */
static struct i2c_adapter * ds3000_i2c_init(struct platform_device *pdev,
...
static int ds3000_drv_probe(struct platform_device *pdev)
{
    int ret = 0;
    int portid_count;
    ...
    for (portid_count = 0 ; portid_count < VIRTUAL_I2C_PORT_LENGTH ; portid_count++) {
        fpga_data->i2c_adapter[portid_count] = ds3000_i2c_init(pdev, portid_count, VIRTUAL_I2C_BUS_OFFSET);
    }
    ...
    for (portid_count = 0; portid_count < SFF_PORT_TOTAL; portid_count++) {
        ...
        if (i2c_adap) {
            fpga_data->sff_devices[portid_count] = ds3000_sff_init(portid_count);
...
static struct platform_driver ds3000_drv = {
    .probe  = ds3000_drv_probe,
    .remove = __exit_p(ds3000_drv_remove),
    .driver = {
        .name   = DRIVER_NAME,
    },
};
...
static int fpga_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    ds3000_dev = platform_device_register_simple(DRIVER_NAME, -1, NULL, 0);
    platform_driver_register(&ds3000_drv);
    return 0;
...
{
    platform_driver_unregister(&ds3000_drv);
    platform_device_unregister(ds3000_dev);
    fpgafw_exit();
    pci_iounmap(pdev, fpga_dev.data_base_addr);
    pci_release_regions(pdev);
    pci_disable_device(pdev);
...
void ds3000_exit(void)
{
    pci_unregister_driver(&pci_dev_ops);
}

module_init(ds3000_init);
module_exit(ds3000_exit);

MODULE_AUTHOR("Pradchaya P. <pphuhcar@celestica.com>");
MODULE_DESCRIPTION("Celestica ds3000 switchboard driver");

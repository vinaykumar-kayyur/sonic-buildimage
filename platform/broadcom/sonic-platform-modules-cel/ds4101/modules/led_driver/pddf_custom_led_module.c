...
#include "pddf_custom_led_defs.h"
...
LED_OPS_DATA alarm_led_ops_data[1]= {0};
LED_OPS_DATA pwr_led_ops_data[1]= {0};
...
static LED_TYPE get_dev_type(char* name)
{
    ...
    if(strcasecmp(name, "SYS_LED") == 0) {
        ...
        ret = LED_LOC;
    } else if(strcasecmp(name, "ALARM_LED") == 0) {
        ret = LED_ALARM;
    } else if(strcasecmp(name, "PWR_LED") == 0) {
        ret = LED_PWR;
...
ssize_t get_status_led(struct device_attribute *da)
{
    ...
    strlcpy(temp_data.cur_state.color, "None", NAME_SIZE);
    for (state=0; state<MAX_LED_STATUS; state++) {
        ...
        for (j = 0; j < VALUE_SIZE && ops_ptr->data[state].reg_values[j] != 0xff; j++) {
           if ((color_val ^ (ops_ptr->data[state].reg_values[j] << ops_ptr->data[state].bits.pos)) == 0) {
                strlcpy(temp_data.cur_state.color, LED_STATUS_STR[state], NAME_SIZE);
                break;
           }
        }
    }
...
ssize_t store_pddf_data(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    {
        case PDDF_CHAR:
            strlcpy(ptr->addr, buf, NAME_SIZE); // to discard newline char form buf
            if (ptr->addr[strlen(buf)-1] == '\n'){
...
ssize_t store_bits_data(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    strlcpy(bits_ptr->bits, buf, NAME_SIZE); // to discard newline char form buf
    if (bits_ptr->bits[strlen(buf)-1] == '\n'){
...
PDDF_LED_DATA_ATTR(cur_state, pwr_led, S_IWUSR|S_IRUGO, show_pddf_s3ip_data,
            store_pddf_s3ip_data, PDDF_INT_DEC, sizeof(int), NULL);
PDDF_LED_DATA_ATTR(cur_state, alarm_led, S_IWUSR|S_IRUGO, show_pddf_s3ip_data,
            store_pddf_s3ip_data, PDDF_INT_DEC, sizeof(int), NULL);
...
PDDF_LED_DATA_ATTR(cur_state, fantray7_led, S_IWUSR|S_IRUGO, show_pddf_s3ip_data,
            store_pddf_s3ip_data, PDDF_INT_DEC, sizeof(int), (void *)&fantray_led_ops_data[6]);
...
struct attribute* attrs_cur_state[] = {
    ...
    &pddf_dev_cur_state_attr_pwr_led.dev_attr.attr,
    &pddf_dev_cur_state_attr_alarm_led.dev_attr.attr,
    ...
    &pddf_dev_cur_state_attr_fantray7_led.dev_attr.attr,
    NULL,
};

...
int fs_i2c_cpld_read(u8 cpld_addr, u8 reg);
int fs_i2c_cpld_write(unsigned short cpld_addr, u8 reg, u8 value);
...
/* Addresses scanned for fs_i2c_cpld
 */
                                ...
                                struct device_attribute *devattr, char *buf)
{
    ...
    while(i < data->sfp_num)
    {
        ...
        if(cpld_addr == client->addr)
           ...
           value = fs_i2c_cpld_read(cpld_addr, reg);
...
static void fs_i2c_cpld_add_client(struct i2c_client *client)
{
...
static void fs_i2c_cpld_remove_client(struct i2c_client *client)
{
...
static int fs_i2c_cpld_probe(struct i2c_client *client,
...
static void fs_i2c_cpld_remove(struct i2c_client *client)
{
    struct cpld_data *data = i2c_get_clientdata(client);
    ...
    sysfs_remove_group(&client->dev.kobj, &data->group);
    kfree(data->group.attrs);
    fs_i2c_cpld_remove_client(client);
}
...
EXPORT_SYMBOL(fs_i2c_cpld_read);
...
EXPORT_SYMBOL(fs_i2c_cpld_write);


static const struct i2c_device_id fs_i2c_cpld_id[] = {
...
MODULE_DEVICE_TABLE(i2c, fs_i2c_cpld_id);

static struct i2c_driver fs_i2c_cpld_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name = "fs_i2c_cpld",
    },
    .probe		= fs_i2c_cpld_probe,
    .remove	   	= fs_i2c_cpld_remove,
    .id_table     = fs_i2c_cpld_id,
    .address_list = normal_i2c,
};


static int __init fs_i2c_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&fs_i2c_cpld_driver);
}

static void __exit fs_i2c_cpld_exit(void)
{
    i2c_del_driver(&fs_i2c_cpld_driver);
}

module_init(fs_i2c_cpld_init);
module_exit(fs_i2c_cpld_exit);


MODULE_DESCRIPTION("fs_i2c_cpld driver");
MODULE_LICENSE("GPL");

/*
 * A hwmon driver for the FS n9550 32d fan
 *
...
static struct n9550_32d_fan_data *n9550_32d_fan_update_device(struct device *dev);
...
static struct attribute *n9550_32d_fan_attributes[] = {
...
static int n9550_32d_fan_read_value(struct i2c_client *client, u8 reg)
{
    return i2c_smbus_read_byte_data(client, reg);
}

static int n9550_32d_fan_write_value(struct i2c_client *client, u8 reg, u8 value)
{
...
static u8 is_fan_fault(struct n9550_32d_fan_data *data, enum fan_id id)
{
    u8 ret = 1;
                              ...
                              const char *buf, size_t count)
{
    ...
    n9550_32d_fan_write_value(client, 0x33, 0); /* Disable fan speed watch dog */
    n9550_32d_fan_write_value(client, fan_reg[FAN_DUTY_CYCLE_PERCENTAGE], duty_cycle_to_reg_val(value));
    return count;
}
                              ...
                              char *buf)
{
    ...
    struct n9550_32d_fan_data *data = n9550_32d_fan_update_device(dev);
    ssize_t ret = 0;
...
static const struct attribute_group n9550_32d_fan_group = {
    .attrs = n9550_32d_fan_attributes,
};

static struct n9550_32d_fan_data *n9550_32d_fan_update_device(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct n9550_32d_fan_data *data = i2c_get_clientdata(client);
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
        ...
        dev_dbg(&client->dev, "Starting n9550_32d_fan update\n");
        data->valid = 0;

        /* Update fan data
         */
        for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
            int status = n9550_32d_fan_read_value(client, fan_reg[i]);
            if (status < 0) {
                data->valid = 0;
...
static int n9550_32d_fan_probe(struct i2c_client *client,
                                const struct i2c_device_id *dev_id)
{
    struct n9550_32d_fan_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct n9550_32d_fan_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &n9550_32d_fan_group);
    if (status) {
        goto exit_free;
    }
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &n9550_32d_fan_group);
exit_free:
    kfree(data);
exit:
...
static void n9550_32d_fan_remove(struct i2c_client *client)
{
    struct n9550_32d_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id n9550_32d_fan_id[] = {
    { "n9550_32d_fan", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, n9550_32d_fan_id);

static struct i2c_driver n9550_32d_fan_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = DRVNAME,
    },
    .probe        = n9550_32d_fan_probe,
    .remove       = n9550_32d_fan_remove,
    .id_table     = n9550_32d_fan_id,
    .address_list = normal_i2c,
};

module_i2c_driver(n9550_32d_fan_driver);

MODULE_DESCRIPTION("n9550_32d_fan driver");
MODULE_LICENSE("GPL");

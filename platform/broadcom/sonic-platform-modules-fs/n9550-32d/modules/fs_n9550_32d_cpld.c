 ...
 * This includes the:
 *	 FS n9550_32d CPLD1/CPLD2/CPLD3
 *
 * Based on:
 *	pca932d.c from Kumar Gala <galak@kernel.crashing.org>
 * Copyright (C) 2006
 *
 * Based on:
 *	pca932d.c from Ken Harrenstien
...
struct n9550_32d_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id n9550_32d_cpld_id[] = {
    { "n9550_32d_fpga", n9550_32d_fpga },
    { "n9550_32d_cpld1", n9550_32d_cpld1 },
    { "n9550_32d_cpld2", n9550_32d_cpld2 },
    { "n9550_32d_cpld_cpu", n9550_32d_cpld_cpu },
    { }
};
MODULE_DEVICE_TABLE(i2c, n9550_32d_cpld_id);
...
enum n9550_32d_cpld_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
...
static ssize_t set_mode_reset(struct device *dev, struct device_attribute *da,
			const char *buf, size_t count);
static int n9550_32d_cpld_read_internal(struct i2c_client *client, u8 reg);
static int n9550_32d_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *n9550_32d_fpga_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	NULL
};

static const struct attribute_group n9550_32d_fpga_group = {
	.attrs = n9550_32d_fpga_attributes,
};

static struct attribute *n9550_32d_cpld1_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
...
static const struct attribute_group n9550_32d_cpld1_group = {
	.attrs = n9550_32d_cpld1_attributes,
};

static struct attribute *n9550_32d_cpld2_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,	
...
static const struct attribute_group n9550_32d_cpld2_group = {
	.attrs = n9550_32d_cpld2_attributes,
};
             ...
             char *buf)
{
    ...
    struct n9550_32d_cpld_data *data = i2c_get_clientdata(client);
    int status = 0;
    u8 reg = 0; 
    ...
    status = n9550_32d_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
    ...
    status = n9550_32d_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0)) {
		goto exit;
	}
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = n9550_32d_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void n9550_32d_cpld_add_client(struct i2c_client *client)
{
...
static void n9550_32d_cpld_remove_client(struct i2c_client *client)
{
...
{    
	...
	status = n9550_32d_cpld_write_internal(client, reg, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
 ...
 */
static int n9550_32d_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct n9550_32d_cpld_data *data;
	int ret = -ENODEV;
	int status;	
	...
	data = kzalloc(sizeof(struct n9550_32d_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    switch (data->type) {
    case n9550_32d_fpga:
        group = &n9550_32d_fpga_group;
        break;
    case n9550_32d_cpld1:
        group = &n9550_32d_cpld1_group;
        /*Set interrupt mask to 0, and then can get intr from 0x8*/
        status=n9550_32d_cpld_write_internal(client, 0x9, 0x0); 
        if (status < 0)
        {
           dev_dbg(&client->dev, "cpld1 reg 0x9 err %d\n", status);            
        } 
        break;
    case n9550_32d_cpld2:
        group = &n9550_32d_cpld2_group;
        /*Set interrupt mask to 0, and then can get intr from 0x8*/
        status=n9550_32d_cpld_write_internal(client, 0x9, 0x0); 
        if (status < 0)
        {
         ...
         break; 
     case n9550_32d_cpld_cpu:
         /* Disable CPLD reset to avoid DUT will be reset.
          */
         status=n9550_32d_cpld_write_internal(client, 0x3, 0x0); 
         if (status < 0)
         {
    ...
    n9550_32d_cpld_add_client(client);
    return 0;
...
static void n9550_32d_cpld_remove(struct i2c_client *client)
{
    struct n9550_32d_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    n9550_32d_cpld_remove_client(client);
...
int n9550_32d_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = n9550_32d_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(n9550_32d_cpld_read);

int n9550_32d_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = n9550_32d_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(n9550_32d_cpld_write);

static struct i2c_driver n9550_32d_cpld_driver = {
	.driver		= {
		.name	= "n9550_32d_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= n9550_32d_cpld_probe,
	.remove		= n9550_32d_cpld_remove,
	.id_table	= n9550_32d_cpld_id,
};

static int __init n9550_32d_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&n9550_32d_cpld_driver);
}

static void __exit n9550_32d_cpld_exit(void)
{
    i2c_del_driver(&n9550_32d_cpld_driver);
}

MODULE_DESCRIPTION("FS I2C CPLD driver");
MODULE_LICENSE("GPL");

module_init(n9550_32d_cpld_init);
module_exit(n9550_32d_cpld_exit);

/*
 * An hwmon driver for the FS Redundant Power Module
 *
 */
...
#define DRIVER_DESCRIPTION_NAME "fs i2c psu driver"
/* PMBus Protocol. */
...
static int fs_i2c_psu_write_word(struct i2c_client *client, u8 reg, u16 value);
static struct fs_i2c_psu_data *fs_i2c_psu_update_device(struct device *dev);

enum fs_i2c_psu_sysfs_attributes {
    PSU_V_IN,
    PSU_V_OUT,
    PSU_I_IN,
    PSU_I_OUT,
...
static struct attribute *fs_i2c_psu_attributes[] = {
			...
			const char *buf, size_t count)
{
    ...
    struct fs_i2c_psu_data *data = i2c_get_clientdata(client);
    ...
    fs_i2c_psu_write_word(client, PMBUS_REGISTER_FAN_COMMAND_1 + nr, data->fan_duty_cycle[nr]);
...
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct fs_i2c_psu_data *data = fs_i2c_psu_update_device(dev);
...
static const struct attribute_group fs_i2c_psu_group = {
    .attrs = fs_i2c_psu_attributes,
};

static int fs_i2c_psu_probe(struct i2c_client *client,
            const struct i2c_device_id *dev_id)
{
    struct fs_i2c_psu_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct fs_i2c_psu_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &fs_i2c_psu_group);
    if (status) {
        goto exit_free;
    }
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &fs_i2c_psu_group);
exit_free:
    kfree(data);
exit:
...
static void fs_i2c_psu_remove(struct i2c_client *client)
{
    struct fs_i2c_psu_data *data = i2c_get_clientdata(client);
...
/* Support psu moduel
 */
static const struct i2c_device_id fs_i2c_psu_id[] = {
    { "acbel_fsh082", 0 },
    { "yesm1300am", 1 },
    {}
};
MODULE_DEVICE_TABLE(i2c, fs_i2c_psu_id);

static struct i2c_driver fs_i2c_psu_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = "fs_i2c_psu",
    },
    .probe        = fs_i2c_psu_probe,
    .remove       = fs_i2c_psu_remove,
    .id_table     = fs_i2c_psu_id,
    .address_list = normal_i2c,
};

static int fs_i2c_psu_read_byte(struct i2c_client *client, u8 reg)
{
    return i2c_smbus_read_byte_data(client, reg);
}

static int fs_i2c_psu_read_word(struct i2c_client *client, u8 reg)
{
...
static int fs_i2c_psu_read_block(struct i2c_client *client, u8 command, u8 *data,
			  int data_len)
{
...
static int fs_i2c_psu_read_block_data(struct i2c_client *client, u8 command, u8 *data, int data_length)
{
    int status = -EIO;
    int length;
    u8 buffer[128] = {0}, *ptr = buffer;

    status = fs_i2c_psu_read_byte(client, command);
    if (status < 0)
    {
    ...
    length = status;
    status = fs_i2c_psu_read_block(client, command, buffer, length);
    if (unlikely(status < 0))
        goto EXIT_READ_BLOCK_DATA;
...
static struct fs_i2c_psu_data *fs_i2c_psu_update_device(struct device *dev)
{
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2)
        ...
        dev_dbg(&client->dev, "Starting fs_i2c_psu update\n");

        /* Read byte data */        
        for (i = 0; i < ARRAY_SIZE(regs_byte); i++) {
            status = fs_i2c_psu_read_byte(client, regs_byte[i].reg);
        ...
        for (i = 0; i < ARRAY_SIZE(regs_word); i++) {
            status = fs_i2c_psu_read_word(client, regs_word[i].reg);
        ...
        /* Read mfr_id */
		status = fs_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_ID, data->mfr_id,
		...
		/* Read mfr_model */		
		status = fs_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_MODEL, data->mfr_model,
        ...
        /* Read mfr_revsion */		
		status = fs_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_REVISION, data->mfr_revsion,
		...
		/* Read mfr_serial */
		status = fs_i2c_psu_read_block_data(client, PMBUS_REGISTER_MFR_SERIAL, data->mfr_serial,
...
static int __init fs_i2c_psu_init(void)
{
    return i2c_add_driver(&fs_i2c_psu_driver);
}

static void __exit fs_i2c_psu_exit(void)
{
    i2c_del_driver(&fs_i2c_psu_driver);
}
...
module_init(fs_i2c_psu_init);
module_exit(fs_i2c_psu_exit);

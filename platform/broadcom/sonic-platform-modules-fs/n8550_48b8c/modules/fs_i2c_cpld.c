 ...
 * This includes the:
 *	 FS n8550_48b8c CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
enum n8550_48b8c_cpld_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
             ...
             char *buf)
{
    ...
    if (data->type == n8550_48b8c_cpld2) {
        regs_p = regs_h1;
    } else {
        regs_p = regs_h2;
    }
    ...
    for (i = 0; i < sizeof(values); i++) {
        status = n8550_48b8c_cpld_read_internal(client, regs_p[i]);
...
static int n8550_48b8c_cpld_remove(struct i2c_client *client)
{
    struct n8550_48b8c_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

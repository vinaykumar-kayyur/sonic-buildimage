...
if $use_57_eeprom ; then
    ...
    if [ -f /usr/share/sonic/device/x86-fs_n8550_48b8c/pddf_support ] && \
       [ -f /usr/share/sonic/device/x86-fs_n8550_48b8c/pddf/pddf-device.json ]; then
        sed -i 's@"topo_info": {"parent_bus": "0x0", "dev_addr": "0x56", "dev_type": "24c04"},@\
            "topo_info": {"parent_bus": "0x0", "dev_addr": "0x57", "dev_type": "24c02"},@g' \
            /usr/share/sonic/device/x86-fs_n8550_48b8c/pddf/pddf-device.json
        sync
    fi
fi

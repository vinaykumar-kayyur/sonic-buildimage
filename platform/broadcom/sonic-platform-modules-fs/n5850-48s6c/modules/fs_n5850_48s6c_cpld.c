 ...
 * 
 *
 * This module supports the fs cpld that hold the channel select
 * mechanism for other i2c slave devices, such as SFP.
 * This includes the:
 *	 FS n5850_48s6c CPLD1/CPLD2/CPLD3
 *
 * Based on:
 *	pca948s6c.c from Kumar Gala <galak@kernel.crashing.org>
 * Copyright (C) 2006
 *
 * Based on:
 *	pca948s6c.c from Ken Harrenstien
...
struct n5850_48s6c_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id n5850_48s6c_cpld_id[] = {
    { "n5850_48s6c_cpld1", n5850_48s6c_cpld1 },
    { "n5850_48s6c_cpld2", n5850_48s6c_cpld2 },
    { "n5850_48s6c_cpld3", n5850_48s6c_cpld3 },
    { }
};
MODULE_DEVICE_TABLE(i2c, n5850_48s6c_cpld_id);
...
enum n5850_48s6c_cpld1_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
             char *buf);
static int n5850_48s6c_cpld_read_internal(struct i2c_client *client, u8 reg);
static int n5850_48s6c_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *n5850_48s6c_cpld1_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	NULL
};

static const struct attribute_group n5850_48s6c_cpld1_group = {
	.attrs = n5850_48s6c_cpld1_attributes,
};

static struct attribute *n5850_48s6c_cpld2_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
...
static const struct attribute_group n5850_48s6c_cpld2_group = {
	.attrs = n5850_48s6c_cpld2_attributes,
};

static struct attribute *n5850_48s6c_cpld3_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
	&sensor_dev_attr_module_present_all.dev_attr.attr,
...
static const struct attribute_group n5850_48s6c_cpld3_group = {
	.attrs = n5850_48s6c_cpld3_attributes,
};
             ...
             char *buf)
{
	...
	struct n5850_48s6c_cpld_data *data = i2c_get_clientdata(client);

	mutex_lock(&data->update_lock);

    for (i = 0; i < size[data->type]; i++) {
        status = n5850_48s6c_cpld_read_internal(client, regs[data->type][i]);
        if (status < 0) {
            goto exit;
        }
    ...
    if (data->type == n5850_48s6c_cpld2) {
        values[4] &= 0x3F;
        return sprintf(buf, "%.2x %.2x %.2x %.2x %.2x\n",
                             values[0], values[1], values[2], values[3], values[4]);
    }
    else { /* n5850_48s6c_cpld3 */
        values[1] &= 0x3;
             ...
             char *buf)
{
    ...
    mutex_lock(&data->update_lock);
	status = n5850_48s6c_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
    ...
    status = n5850_48s6c_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0)) {
		goto exit;
	}
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = n5850_48s6c_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void n5850_48s6c_cpld_add_client(struct i2c_client *client)
{
...
static void n5850_48s6c_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int n5850_48s6c_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct n5850_48s6c_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct n5850_48s6c_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    switch (data->type) {
    case n5850_48s6c_cpld1:
        group = &n5850_48s6c_cpld1_group;
        break;
    case n5850_48s6c_cpld2:
        group = &n5850_48s6c_cpld2_group;
        break;
	case n5850_48s6c_cpld3:
        group = &n5850_48s6c_cpld3_group;
        break;
    default:
        break;
    }
    ...
    n5850_48s6c_cpld_add_client(client);
    return 0;
...
static void n5850_48s6c_cpld_remove(struct i2c_client *client)
{
    struct n5850_48s6c_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    n5850_48s6c_cpld_remove_client(client);
...
int n5850_48s6c_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = n5850_48s6c_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(n5850_48s6c_cpld_read);

int n5850_48s6c_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = n5850_48s6c_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(n5850_48s6c_cpld_write);

static struct i2c_driver n5850_48s6c_cpld_driver = {
	.driver		= {
		.name	= "n5850_48s6c_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= n5850_48s6c_cpld_probe,
	.remove		= n5850_48s6c_cpld_remove,
	.id_table	= n5850_48s6c_cpld_id,
};

static int __init n5850_48s6c_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&n5850_48s6c_cpld_driver);
}

static void __exit n5850_48s6c_cpld_exit(void)
{
    i2c_del_driver(&n5850_48s6c_cpld_driver);
}

MODULE_DESCRIPTION("FS I2C CPLD driver");
MODULE_LICENSE("GPL");

module_init(n5850_48s6c_cpld_init);
module_exit(n5850_48s6c_cpld_exit);

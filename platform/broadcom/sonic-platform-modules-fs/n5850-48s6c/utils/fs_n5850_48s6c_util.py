#!/usr/bin/env python3
#
# Copyright (C) 2019 FS Networks, Inc.
#
...
def driver_check():
    ret, lsmod = log_os_system("ls /sys/module/*fs*", 0)
...
'modprobe i2c_i801',
'modprobe i2c_ismt',
'modprobe i2c_mux_pca948s6c force_deselect_on_exit=1',
'modprobe fs_n5850_48s6c_cpld'  ,
'modprobe ym2651y'                  ,
'modprobe fs_n5850_48s6c_fan'     ,
'modprobe optoe'      ,
'modprobe fs_n5850_48s6c_leds'      ,
'modprobe fs_n5850_48s6c_psu' ]
...
'echo n5850_48s6c_fan 0x63 > /sys/bus/i2c/devices/i2c-3/new_device',
...
'echo n5850_48s6c_psu1 0x50 > /sys/bus/i2c/devices/i2c-11/new_device',
'echo ym2401 0x58 > /sys/bus/i2c/devices/i2c-11/new_device',
'echo n5850_48s6c_psu2 0x53 > /sys/bus/i2c/devices/i2c-12/new_device',
'echo ym2401 0x5b > /sys/bus/i2c/devices/i2c-12/new_device',
'echo n5850_48s6c_cpld1 0x60 > /sys/bus/i2c/devices/i2c-3/new_device',
'echo n5850_48s6c_cpld2 0x61 > /sys/bus/i2c/devices/i2c-3/new_device',
'echo n5850_48s6c_cpld3 0x62 > /sys/bus/i2c/devices/i2c-3/new_device']
...
def device_install():
    ...
    if order:
        for i in range(0,len(mknod2)):
            #for pca948s6c need times to built new i2c buses
    ...
    # set all pca948s6c idle_disconnect
    cmd = 'echo -2 | tee /sys/bus/i2c/drivers/pca948s6c/*-00*/idle_state'
...
def do_sonic_platform_install():
    device_path = "{}{}{}{}".format(PLATFORM_ROOT_PATH, '/x86_64-fs_', PROJECT_NAME, '-r0')

...
# via current method (sai.profile.j2) or new method (append.config.j2)
# If delta is large, use sai.profile.j2 which basically require the user to select which config file to use
# If delta is small, use append.config.j2 where additional SAI INIT config properties are added
#   based on specific device metadata requirement
#   in this case sai.profile should have been modified to use the path /etc/sai.d/sai.config
...
if [ -f $HWSKU_DIR/sai.profile.j2 ]; then
    ...
    if [ -f $HWSKU_DIR/append.config.j2 ]; then
        sonic-cfggen -d -t $HWSKU_DIR/append.config.j2 > /etc/sai.d/sai.config
    fi

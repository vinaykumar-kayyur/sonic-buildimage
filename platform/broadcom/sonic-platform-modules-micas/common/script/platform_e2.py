...
def wedge_v5_eeprom_show(e2_name, eeprom, e2_decode=None):
    try:
        ...
        print("===================%s===================" % e2_name)
        print("%-32s %-5s %-5s  %-20s" % ("TLV name","Type","Length","Value"))
        for item in rets:
            print("%-32s %-5d   %-5s %-20s" % (item["name"],item["code"],item["lens"],item["value"]))
        return True, ""
    except Exception as e:
        return False, str(e)
...
@click.pass_context
def main(ctx, parse, file, type, size):
    '''platform eeprom display script'''
    if ctx.invoked_subcommand is None and parse is None and file is None and type is None:

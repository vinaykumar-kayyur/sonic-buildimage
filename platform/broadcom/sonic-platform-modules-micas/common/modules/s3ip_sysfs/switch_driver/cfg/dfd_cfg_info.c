...
int dfd_info_set_int(int key, int val)
{
    ...
    if (IS_INFO_FRMT_BIT(info_ctrl->frmt)) {
        ...
        if (write_bytes <= 0) {
            DBG_DEBUG(DBG_ERROR, "info ctrl[key=0x%08x] write num_str: fpath: %s, len[%d] invalid\n",
                key, info_ctrl->fpath, write_bytes);
            return -DFD_RV_INVALID_VALUE;
        }
        bit_mask = 0xff;
        DBG_DEBUG(DBG_VERBOSE, "info ctrl[key=0x%08x], write num_str: fpath: %s, write val: %s, len: %d\n",
...
static long dfd_info_reg2data_linear(int key, int data)
{
    s16 exponent;
    s32 mantissa;
    long val;
    ...
    info_ctrl = dfd_ko_cfg_get_item(key);
    if (info_ctrl == NULL) {
        DBG_DEBUG(DBG_WARN, "get info ctrl fail, key=0x%d\n", key);
        return -DFD_RV_DEV_NOTSUPPORT;
    }
...
static long dfd_info_reg2data_tmp464(int key, int data)
{
    s16 tmp_val;
    long val;
...
static long dfd_info_reg2data_mac_th5(int key, int data)
{
    int tmp_val;
    long val;
...
static long dfd_info_reg2data_mac_th4(int key, int data)
{
    int tmp_val;
    int val;
...
int dfd_info_get_sensor(uint32_t key, char *buf, int buf_len, info_hwmon_buf_f pfun)
{
    ...
    if (key_info_ctrl->mode == INFO_CTRL_MODE_CFG && key_info_ctrl->src == INFO_SRC_FILE) {
        if (strstr(key_info_ctrl->fpath, "hwmon") != NULL) {
            ...
            DBG_DEBUG(DBG_VERBOSE, "get sensor value, key:0x%08x\n", key);
            rv = dfd_info_get_buf(key, buf, buf_len, NULL);
            if (rv < 0) {
                DBG_DEBUG(DBG_VERBOSE, "get sensor value failed, key:0x%08x, rv:%d\n", key, rv);
            }
            return rv;
        }
    }

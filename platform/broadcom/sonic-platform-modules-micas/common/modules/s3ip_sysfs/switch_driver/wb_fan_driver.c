 ...
 *
 * Read and write functions of fan-related attributes
 * History
...
static int dfd_get_fan_motor_speed_tolerance(unsigned int fan_index, unsigned int motor_index, int *value)
{
    ...
    *value = target * *p_fan_speed_tolerance / 100;
    ...
    DFD_FAN_DEBUG(DBG_VERBOSE, "get fan%u motor%u speed tolerance ok, key: 0x%x, tolerance rate: %d, value: %d\n",
        fan_index, motor_index, key, *p_fan_speed_tolerance, *value);
    return DFD_RV_OK;
}
...
static int dfd_get_fan_direction(unsigned int fan_index, int *value)
{
    ...
    if (p_fan_dirction == NULL) {
        DFD_FAN_DEBUG(DBG_ERROR, "get fan%u direction failed, key: 0x%x\n",
            fan_index, key);
        return -DFD_RV_DEV_NOTSUPPORT;
    }
    *value = *p_fan_dirction;
    DFD_FAN_DEBUG(DBG_VERBOSE, "get fan%u direction success, key: 0x%x, value: %d\n",
        fan_index, key, *value);
    return DFD_RV_OK;
}

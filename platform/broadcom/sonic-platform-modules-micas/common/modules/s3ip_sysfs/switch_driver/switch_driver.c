...
#include "./include/dfd_sysfs_common.h"
#include "./include/switch_driver.h"
#include "./include/wb_module.h"
#include "./include/wb_fan_driver.h"
#include "./include/wb_eeprom_driver.h"
#include "./include/wb_cpld_driver.h"
#include "./include/wb_fpga_driver.h"
#include "./include/wb_led_driver.h"
#include "./include/wb_slot_driver.h"
#include "./include/wb_sensors_driver.h"
#include "./include/wb_psu_driver.h"
#include "./include/wb_sff_driver.h"
#include "./include/wb_watchdog_driver.h"
...
static ssize_t dfd_get_fan_model_name(unsigned int fan_index, char *buf, size_t count)
{
    ...
    ret = dfd_get_fan_status(fan_index, buf, count);
    if (ret < 0) {
        return ret;
    }
    if ((int32_t)simple_strtol(buf, NULL, 10) == FAN_STATUS_ABSENT) {
        memset(buf, 0, count);
...
static ssize_t dfd_get_psu_model_name(unsigned int psu_index, char *buf, size_t count)
{
    ...
    ret = dfd_get_psu_present_status(psu_index, buf, count);
    if (ret < 0) {
        return ret;
    }
    if ((int32_t)simple_strtol(buf, NULL, 10) == PSU_STATUS_ABSENT) {
        memset(buf, 0, count);
        return (ssize_t)snprintf(buf, count, "%s\n", SWITCH_DEV_NO_SUPPORT);
    }
...
static ssize_t dfd_get_psu_status(unsigned int psu_index, char *buf, size_t count)
{
    ...
    status_word = dfd_get_psu_pmbus_status(psu_index);
    if (status_word < 0) {
        return status_word;
    } else {
...
static ssize_t dfd_get_transceiver_power_on_status(char *buf, size_t count)
{
    ...
    ret = dfd_get_sff_cpld_info(0, WB_SFF_POWER_ON, buf, count);
    return ret;
}
...
{
    /* add vendor codes here */
    return dfd_watchdog_set_status(value);
}
...
/***************************************end of slot*******************************************/
/*****************************************system*********************************************/
static ssize_t dfd_get_system_value(unsigned int type, int *value)
{
    int ret;

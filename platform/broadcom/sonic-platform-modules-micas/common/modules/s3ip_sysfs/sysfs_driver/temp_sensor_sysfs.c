...
#include "./include/temp_sensor_sysfs.h"
...
static ssize_t temp_sensor_high_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    temp_index = obj->index;
    TEMP_SENSOR_DBG("temp index: %u\n", temp_index);
    ret = g_temp_sensor_drv->get_main_board_temp_high(temp_index, buf, PAGE_SIZE);
    if (ret < 0) {
        TEMP_SENSOR_ERR("get temp%u high threshold failed, ret: %d\n", temp_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}
...
static ssize_t temp_sensor_low_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    temp_index = obj->index;
    TEMP_SENSOR_DBG("temp index: %u\n", temp_index);
    ret = g_temp_sensor_drv->get_main_board_temp_low(temp_index, buf, PAGE_SIZE);
    if (ret < 0) {
        TEMP_SENSOR_ERR("get temp%u low threshold failed, ret: %d\n", temp_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

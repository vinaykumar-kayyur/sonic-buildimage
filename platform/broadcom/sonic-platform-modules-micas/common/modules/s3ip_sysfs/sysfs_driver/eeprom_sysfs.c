 ...
 * eeprom_sysfs.c
 *
 * This module create eeprom kobjects and attributes in /sys/s3ip/eeprom
 *
 * History
...
#include "./include/eeprom_sysfs.h"
...
static ssize_t eeprom_alias_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    ret = g_eeprom_drv->get_eeprom_alias(eeprom_index, buf, PAGE_SIZE);
    if (ret < 0) {
        EEPROM_ERR("get eeprom%u alias failed, ret: %d\n", eeprom_index, ret);
...
static ssize_t eeprom_tag_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    ret = g_eeprom_drv->get_eeprom_tag(eeprom_index, buf, PAGE_SIZE);
    if (ret < 0) {
        EEPROM_ERR("get eeprom%u tag failed, ret: %d\n", eeprom_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}
...
static ssize_t eeprom_type_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    ...
    ret = g_eeprom_drv->get_eeprom_type(eeprom_index, buf, PAGE_SIZE);
    if (ret < 0) {
        EEPROM_ERR("get eeprom%u type failed, ret: %d\n", eeprom_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}
...
static int eeprom_sub_single_create_kobj(struct kobject *parent, unsigned int index)
{
    ...
    curr_eeprom = &g_eeprom.eeprom[index - 1];
    memset(eeprom_dir_name, 0, sizeof(eeprom_dir_name));
...
int s3ip_sysfs_eeprom_drivers_register(struct s3ip_sysfs_eeprom_drivers_s *drv)
{
    ...
    memset(&g_eeprom, 0, sizeof(struct eeprom_s));

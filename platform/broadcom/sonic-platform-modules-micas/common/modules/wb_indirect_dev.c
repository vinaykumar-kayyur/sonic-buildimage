...
static int wb_dev_file_read(const char *path, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    if (ret < 0) {
        INDIRECT_DEV_ERROR("kernel_read failed, path=%s, addr=0x%x, size=%zu, ret=%d\r\n", path, pos, size, ret);
        goto exit;
    }
...
static int wb_dev_file_write(const char *path, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    if (ret < 0) {
        INDIRECT_DEV_ERROR("kernel_write failed, path=%s, addr=0x%x, size=%zu, ret=%d\r\n", path, pos, size, ret);
        goto exit;
    }
...
static ssize_t indirect_dev_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
    u8 val[MAX_RW_LEN];
    int ret, read_len;
...
static ssize_t indirect_dev_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
    ...
    ret = indirect_dev_read(iocb->ki_filp, to->kvec->iov_base, to->count, &iocb->ki_pos);
    return ret;
}
...
static ssize_t indirect_dev_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
    ...
    ret = indirect_dev_write(iocb->ki_filp, from->kvec->iov_base, from->count, &iocb->ki_pos);
    return ret;
}

...
STATUS_LED_PATH = "/sys/devices/platform/leds:green:stat/brightness"
...
class Chassis(ChassisBase):
    ...
    def __init__(self):
        ...
        self.__initialize_global_port_pre_dict()
    ...
    def __initialize_sfp(self):
        ...
        for index in range(8):
            eeprom_path = eeprom_base.format(index + 9)
            sfp = Sfp((index + 1), port_type, eeprom_path)
            self._sfp_list.append(sfp)
        for index in range(8):
            eeprom_path = eeprom_base.format(index + 18)
            sfp = Sfp((index + 9), port_type, eeprom_path)
            self._sfp_list.append(sfp)
        for index in range(8):
            eeprom_path = eeprom_base.format(index + 27)
            sfp = Sfp((index + 17), port_type, eeprom_path)
            self._sfp_list.append(sfp)
        port_type = 'QSFP_DD'
        for index in range(8):
            eeprom_path = eeprom_base.format(index + 39)
            sfp = Sfp((index + 25), port_type, eeprom_path)
            self._sfp_list.append(sfp)
    ...
    def __initialize_fan(self):
        ...
        for fant_i in range(0, NUM_FAN_TRAY):
            for fan_i in range(0, NUM_FAN):
                fan = Fan(fant_i, fan_i)
                self._fan_list.append(fan)
        """
        from sonic_platform.fan_drawer import FanDrawer
        for fant_i in range(0, NUM_FAN_TRAY):
            fan_dr = FanDrawer(fant_i)
            self._fan_drawer_list.append(fan_dr)
            self._fan_list.extend(fan_dr._fan_list)
    ...
    def __initialize_global_port_pre_dict(self):
        for port_num in range(1, NUM_SFP+1):
    ...
    def get_reboot_cause(self):
        ...
        prev_reboot_cause = {
            ...
            '0x22': (self.REBOOT_CAUSE_NON_HARDWARE, 'CPU warm reset by Software'),
            '0x33': (self.REBOOT_CAUSE_NON_HARDWARE, 'CPU cold reset by software'),
            '0x44': (self.REBOOT_CAUSE_HARDWARE_OTHER, 'CPU warm reboot'),
            '0x55': (self.REBOOT_CAUSE_HARDWARE_OTHER, 'CPU cold reset'),
            '0x66': (self.REBOOT_CAUSE_WATCHDOG, 'Power reset by watchdog'),
            '0x77': (self.REBOOT_CAUSE_NON_HARDWARE, 'Power cycle by Software')

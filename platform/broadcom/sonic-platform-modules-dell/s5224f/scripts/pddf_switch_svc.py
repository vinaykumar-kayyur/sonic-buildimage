...
def stop_platform_svc():
    status, output = subprocess.getstatusoutput("systemctl stop platform-modules-s5224f.service")
    if status:
        print("Stop platform-modules-s5224f.service failed %d"%status)
        return False
    status, output = subprocess.getstatusoutput("systemctl disable platform-modules-s5224f.service")
    if status:
        print("Disable platform-modules-s5224f.service failed %d"%status)
        return False
...
def start_platform_svc():
    status, output = subprocess.getstatusoutput("systemctl enable platform-modules-s5224f.service")
    if status:
        print("Enable platform-modules-s5224f.service failed %d"%status)
        return False
    status, output = subprocess.getstatusoutput("systemctl start platform-modules-s5224f.service")
    if status:
        print("Start platform-modules-s5224f.service failed %d"%status)
        return False

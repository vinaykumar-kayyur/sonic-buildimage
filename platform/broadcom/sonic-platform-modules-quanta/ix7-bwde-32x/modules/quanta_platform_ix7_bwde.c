...
static int __init ix7_bwde_platform_init(void)
{
	...
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[0]);		// pca9546
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[1]);		// pca9548
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[10]);	// CPU Board Data
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x10);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[8]);		// CPLD2
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[12]);		// CPLD_led_1
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x11);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[9]);		// CPLD3
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[13]);		// CPLD_led_2
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x12);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[2]);		// MB_BOARDINFO_EEPROM
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x13);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[7]);		// pca9555 MB Board Data
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x14);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[3]);		// pca9548_1 SFP
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x15);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[4]);		// pca9548_2 SFP
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x16);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[5]);		// pca9548_3 SFP
	i2c_put_adapter(adapter);

	adapter = i2c_get_adapter(0x17);
	client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[6]);		// pca9548_4 SFP
	i2c_put_adapter(adapter);
	for(i = 32; i < 64; i ++){									// QSFP 1~32 EEPROM
		adapter = i2c_get_adapter(i);
		client = i2c_new_device(adapter, &ix7_bwde_i2c_devices[11]);
		i2c_put_adapter(adapter);
	}

...
static bool int_flag_monitor_en = true;
...
static void sff_fsm_state_change_process(int port, sff_fsm_state_t cur_st, sff_fsm_state_t next_st);
...
struct sff_fsm_t *Sff_Fsm = NULL;

struct qsfp_dd_privat_data {
...
struct qsfp_privat_data {
    u8 lane_st[CH_STATUS_NUM];
};

union private_data_t {

    struct qsfp_dd_privat_data qsfp_dd;
    struct qsfp_privat_data qsfp;
};
struct sff_kobj_t
{
    struct kobject kobj; 
...
struct sff_obj_t 
{
    ...
    union private_data_t private_data;
};   
...
static inline bool transvr_is_detected(int port)
{
    return (Sff_Fsm[port].st == SFF_FSM_ST_DETECTED ? (true) : (false));  

}    
static inline void transvr_type_set(int port, int type)
{
   Sff.obj[port].transvr_type=type; 
}    

static inline int transvr_type_get(int port)
{
   return Sff.obj[port].transvr_type; 
}    
//static int transvr_is_prsL(int port);
...
static int sfp_eeprom_read(int port, u8 slave_addr, u8 offset, u8 *buf, int len);
static int sfp_eeprom_write(int port,u8 slave_addr, u8 offset, u8 *buf, int len);
static int qsfp_eeprom_read(int port, u8 page, u8 offset, u8 *buf, int len);
static int qsfp_eeprom_write(int port, u8 page, u8 offset, u8 *buf, int len);
/*mgr function declaration*/

static int lpmode_set(int port, u8 value);
static int lpmode_get(int port, u8 *value);
static int mode_sel_set(int port, u8 value);
static int mode_sel_get(int port, u8 *value);
static int tx_disable_set(int port, u8 value);
static int tx_disable_get(int port, u8 *value);
static int qsfp_temperature_get(int port, u8 *buf, int buf_size);
static int qsfp_voltage_get(int port, u8 *buf, int buf_size);
static int qsfp_cdr_control(int port);
static int qsfp_lpmode_control(int port);
static int qsfp_tx_disable_set(int port, u8 value);
static int qsfp_tx_disable_get(int port, u8 *value);
static int qsfp_intL_get(int port, u8 *value);
static int sfp_tx_disable_set(int port, u8 value);
static int sfp_tx_disable_get(int port, u8 *value);

static int qsfp_channel_control_set(int port, int type, u32 value);
static int qsfp_channel_control_get(int port, int type, u32 *value);
static int qsfp_channel_monitor_get(int port, int type, u8 *buf, int buf_size);
static int qsfp_vendor_info_get(int port, int type, u8 *buf, int buf_size);
static int qsfp_channel_status_get(int port, int type, u8 *value);
static int sfp_channel_status_get(int port, int type, u8 *value);
int qsfp_lane_status_get(int port, int type, u8 *st);
int qsfp_lane_status_update(int port, int type, u8 value);
//static int qsfp_int_flag_read(int port);

static int sfp_channel_control_set(int port, int type, u32 value);
static int sfp_channel_control_get(int port, int type, u32 *value);
/*qsfp_dd function*/
static bool is_bank_num_valid(int port);
static int qsfp_dd_channel_control_get(int port, int type, u32 *value);
static int qsfp_dd_active_control_set_indicator(int port);
//static int qsfp_dd_id_status_get(int port, struct qsfp_dd_id_status_t *attrta);
static int qsfp_dd_lane_monitor_get(int port, int type, u8 *buf, int buf_size);
static int qsfp_dd_tx_disable_set(int port, u8 val);
static int qsfp_dd_tx_disable_get(int port, u8 *val);
static int qsfp_dd_id_get(int port, u8 *val);
static int qsfp_dd_vendor_info_get(int port, int type, u8 *buf, int buf_size);
//static int qsfp_dd_force_low_pwr(int port, int en);
//static int app_advert_field_dump(int port, u8 *buf, int buf_size);
static int qsfp_dd_module_st_get(int port, u8 *st);
static int qsfp_dd_temperature_get(int port, u8 *buf, int buf_size);
static int qsfp_dd_voltage_get(int port, u8 *buf, int buf_size);
//static int qsfp_dd_interrupt_flags_read(int port);
/*utility*/
...
{
    int type;
    int (*read)(int , u8, u8,u8 *, int);
    int (*write)(int , u8, u8,u8 *, int);   
};
...
static inline int sff_fsm_st_get(int port);
static inline void sff_fsm_st_set(int port, sff_fsm_state_t st);
static void sff_fsm_delay_cnt_reset(int port, sff_fsm_state_t st);
static void sff_fsm_cnt_run(int port);
static bool sff_fsm_delay_cnt_is_hit(int port);

static int sff_fsm_sfp_task(int port);
static int sff_fsm_qsfp_task(int port);

static int sff_fsm_run(void);
static int sff_fsm_create(int size);
static int sff_fsm_destroy(void);

static int valid_lane_num_get(int port);
//static int debug_eeprom_access(int port, int type, const char *str, int len);
static inline bool transvr_is_isolated(int port)
{
    return (SFF_FSM_ST_FAULT == sff_fsm_st_get(port) ? (true) : (false));
}
static inline unsigned long prsL_bitmap_get(void)
{
    return Sff.prsL; 
}    
static inline void prsL_bitmap_update(unsigned long bitmap)
{
   Sff.prsL = bitmap; 
}
...
static int prsL_get(int port, u8 *prsL)
{
    if(IS_ERR_OR_NULL(Sff.drv))
    {
        //<TBD>
        return -1;
    }    
    return Sff.drv->prsL_get(port, prsL);
}
...
static int sfp_eeprom_read(int port,
                            u8 slave_addr,
                            u8 offset,
                            u8 *buf,
                            int len)
{
    ...
    return Sff.drv->eeprom_read(port, slave_addr, offset, buf, len);
}
static int sfp_eeprom_write(int port,
                            u8 slave_addr,
                            u8 offset,
                            u8 *buf,
                            int len)
{
    ...
    return Sff.drv->eeprom_write(port, slave_addr, offset, buf, len);
}
static int _page_switch(int port, u8 page, u8 offset) 
{
    ...
    if (offset >= 128 &&
        page != QSFP_PAGE0_LOWER)
     {
         /*need to switch page*/
         reg_page = page;
         rec = Sff.drv->eeprom_write(port, slave_addr, offset_page_sel, &reg_page, 1);
         if (rec < 0)
         {
            SFF_MGR_DEBUG("switch page fail\n");
            return rec;
         }
     }         
...
static int qsfp_eeprom_read(int port,
                             u8 page,
                             u8 offset,
                             u8 *buf,
                             int len)
{
    int ret = 0;
    u8 slave_addr = SFF_EEPROM_I2C_ADDR;
    ret = _page_switch(port, page, offset); 
    if (ret < 0) {
        return ret;
    }
...
static int qsfp_eeprom_write(int port,
                             u8 page,
                             u8 offset,
                             u8 *buf,
                             int len)
...
static int _sfp_id_read(int port, u8 *id)
{
    u8 slave_addr = 0xff;
    u8 offset = 0;
    int len = 0;
    get_sfp_field_addr(IDENTIFIER, &slave_addr, &offset, &len);
    return sfp_eeprom_read(port, slave_addr , offset, id, len);
}
static int _qsfp_id_read(int port, u8 *id)
{
    u8 page = 0xff;
    u8 offset = 0;
    int len = 0;
    get_qsfp_field_addr(IDENTIFIER, &page, &offset, &len);
    return qsfp_eeprom_read(port, page, offset, id, len);
}
static int _sfp_data_ready_check(int port, u8 *is_ready)
{
    u8 slave_addr = 0xff;
    u8 offset = 0;
    int data_len = 0;
    ...
    if((ret = sfp_eeprom_read(port, slave_addr, offset, &status, data_len)) < 0)
    {
        return ret;
    ...
    *is_ready = status;
    ...
    return 0;
    #if 0  
    SFF_MGR_INFO("special_case of checking data ready\n");
    ...
    if(sfp_eeprom_read(port, slave_addr, offset, &status, data_len) < 0) {
        goto exit_err;
     }    
    *is_ready = 1;
...
static int _qsfp_data_ready_check(int port, u8 *is_ready)
{
    u8 page = 0xff;
    u8 offset = 0;
    int data_len = 0;
    ...
    if((ret = qsfp_eeprom_read(port, page, offset, status, data_len)) < 0)
    {
    ...
    *is_ready = val;
    ...
    SFF_MGR_INFO("special_case of checking data ready\n");
    if((ret = _sfp_id_read(port, &val)) < 0) {
        return ret;
    }
    *is_ready = 1;
    return 0;
...
static int _qsfp_ethernet_compliance_read(int port, u8 *eth_comp)
{
    u8 page = 0xff;
    u8 offset = 0;
    int data_len = 0;
    ...
    if(qsfp_eeprom_read(port, page, offset, &data, data_len) < 0)
    {
...
static int _sfp_transvr_codes_read(int port, u8 *transvr_codes, int size)
{
    u8 slave_addr = 0xff;
    u8 offset = 0;
    int data_len = 0;
    ...
    if(sfp_eeprom_read(port, slave_addr, offset, transvr_codes, data_len) < 0)
    {
        SFF_MGR_ERR("qsfp_eeprom_read fail");
        return -1;
    }
...
static int _sfp_eth_extended_compliance_get(int port, u8 *eth_ext_comp)
{
    u8 slave_addr = 0xff;
    u8 offset = 0;
    int data_len = 0;
    ...
    if(sfp_eeprom_read(port, slave_addr, offset, &data, data_len) < 0)
    {
...
static int _qsfp_eth_extended_compliance_get(int port, u8 *eth_ext_comp)
{
    u8 page = 0xff;
    u8 offset = 0;
    int data_len = 0;
...
static int _sfp_connector_type_read(int port, u8 *conn_type)
{
    u8 slave_addr = 0xff;
    u8 offset = 0;
    int data_len = 0;
...
#if 0
static int _qsfp_connector_type_read(int port, u8 *conn_type)
{
    u8 page = 0xff;
    u8 offset = 0;
    int data_len = 0;
 ...
 * SFF-8024 for codes.*/
static int qsfp_dd_connector_type_read(int port, u8 *conn_type)
{
    u8 data = 0;
    ...
    if(qsfp_eeprom_read(port, QSFP_PAGE0_UPPER, 203, &data, 1) < 0)
    {
        SFF_MGR_ERR("qsfp_eeprom_read fail");
...
static int debug_eeprom_access(int port, int type, const char *str, int len)
{
    ...
    if (match(ptr, "set")) {
        ...
        data = tmp;
        SFF_MGR_INFO("addr:%d offset:%d tmp:%d\n", addr_or_page, offset, (int)tmp);
        if(pfunc->write(port, addr_or_page, offset, &data, 1) < 0) {
        ...
        data_len = tmp;
        if(pfunc->read(port, addr_or_page, offset, debug_read_buf, data_len) < 0) {
...
static int sff_type_get(int port)
{
    int type = SFP_TYPE;
    type = Sff.obj[port].type;
    return type;
                       ...
                       char *buf)
{
    ...
    port = port_get(sff_kobj->kobj.name); 
    type = sff_type_get(port);
    if (SFP_TYPE == type)
    {
      ret = _sfp_id_read(port, &id);   
    }
    else if (QSFP_TYPE == type) 
    {
      ret = _qsfp_id_read(port, &id);   
    } else {

      ret = qsfp_dd_id_get(port, &id);   
    }    
                       ...
                       char *buf)
{
    ...
    {
      ret = qsfp_dd_module_st_get(port, &val);   
    }
                                          ...
                                          const char *buf, size_t count)
{
    ...
    if (SFP_TYPE == type)
    {
      ret = sfp_channel_control_set(port, ctrl_type, ch_ctrl);
    }
    else
    {
      ret = qsfp_channel_control_set(port, ctrl_type, ch_ctrl);
      if(ret < 0)
      {
        return ret;
      }        
...
static ssize_t channel_control_show(int ctrl_type, struct sff_kobj_t *sff_kobj, char *buf)
{
    ...
    if (SFP_TYPE == type)
    {
      ret = sfp_channel_control_get(port, ctrl_type, &ch_ctrl); 
    }
    else if (QSFP_TYPE == type)
    {
      ret = qsfp_channel_control_get(port, ctrl_type, &ch_ctrl); 
    }
    else
    {
      ret = qsfp_dd_channel_control_get(port, ctrl_type, &ch_ctrl);  
      ret = qsfp_dd_active_control_set_indicator(port); /*<TBD>testing*/        
    }    
...
static ssize_t channel_status_show(int st_type, struct sff_kobj_t *sff_kobj, char *buf)
{
    ...
    u8 intL = 0;
    if ((ret = qsfp_intL_get(port, &intL)) < 0) {
        goto err_exit;
    }
    if (intL) {
        if ((ret = qsfp_int_flag_read(port)) < 0) {
            goto err_exit;
        }
    }
    ...
    if (SFP_TYPE == type)
    {
      ret = sfp_channel_status_get(port, st_type, &ch_status); 
    }
    else
    {
      ret = qsfp_lane_status_get(port, st_type, &ch_status); 
    }    
    ...
    {
      ret = qsfp_channel_status_get(port, st_type, &ch_status); 
    }    
...
static ssize_t channel_monitor_show(int moni_type, struct sff_kobj_t *sff_kobj, char *buf)
{
    ...
    if (SFP_TYPE == type)
    {
      ...
      ret = qsfp_channel_monitor_get(port, moni_type, buf, BUF_SIZE); 
    } else {

      ret = qsfp_dd_lane_monitor_get(port, moni_type, buf, BUF_SIZE); 
    }    
                       ...
                       char *buf)
{
    ...
    if (SFP_TYPE == type)
    {
      ...
      ret = app_advert_field_dump(port, buf, BUF_SIZE); 
    }    
...
static ssize_t vendor_info_show(int info_type, struct sff_kobj_t *sff_kobj, char *buf)
{
    ...
    {
      ret = qsfp_vendor_info_get(port, info_type, buf, BUF_SIZE); 
    } else {


      ret = qsfp_dd_vendor_info_get(port, info_type, buf, BUF_SIZE); 
    }    
                       ...
                       char *buf)
{
    int port = port_get(sff_kobj->kobj.name);
    int ret = 0;
    int type =  sff_type_get(port);
    if (SFP_TYPE == type) {
    
    } else if (QSFP_TYPE == type) {
      ret = qsfp_temperature_get(port, buf, BUF_SIZE); 
    } else {
      ret = qsfp_dd_temperature_get(port, buf, BUF_SIZE); 
    }    
                       ...
                       char *buf)
{
    ...
    if (SFP_TYPE == type)
    {
        ...
        ret = qsfp_voltage_get(port, buf, BUF_SIZE); 
    } else {
        ret = qsfp_dd_voltage_get(port, buf, BUF_SIZE); 
    }    
                     ...
                     const char *buf, size_t count)
{
    ...
    int ret = 0;
    int resetL = 0;
    int old_st = sff_fsm_st_get(port);
    ...
    if ((ret = Sff.drv->reset_set(port, resetL)) < 0)
    {
        return ret;
    }
    if (!resetL) { 
        sff_fsm_st_set(port, SFF_FSM_ST_RESET_ASSERTED);
    } else {
        sff_fsm_st_set(port, SFF_FSM_ST_RESET_DEASSERTED);
    }
    
    sff_fsm_state_change_process(port, old_st, sff_fsm_st_get(port));    
    transvr_type_set(port, TRANSVR_CLASS_UNKNOWN);    
    return count;
}
                             ...
                             char *buf)
{
    ...
    if (Sff.drv->reset_get(port, &reset) < 0)
    {
        return scnprintf(buf, BUF_SIZE, "%d\n", err_code);
    }
                     ...
                     const char *buf, size_t count)
{
    ...
    int err_code = -1;
    int type = sff_type_get(port);
    err_code = debug_eeprom_access(port, type, buf, count);
                     ...
                     const char *buf, size_t count)
{
    ...
    err_code = Sff.drv->io_mux_reset_all(val);
                     ...
                     const char *buf, size_t count)
{
    ...
    err_code = lpmode_set(port, lpmode);
                     ...
                     const char *buf, size_t count)
{
    ...
    err_code = mode_sel_set(port, mode_sel);
                             ...
                             char *buf)
{
    ...
    int type = sff_type_get(port); 
    if (SFP_TYPE != type) {
        if (mode_sel_get(port, &mode_sel) < 0)
        {
            return scnprintf(buf, BUF_SIZE, "%d\n", err_code);
        }
    } else {
                     ...
                     const char *buf, size_t count)
{
    ...
    err_code = tx_disable_set(port, tx_disable);
                             ...
                             char *buf)
{
    ...
    if (tx_disable_get(port, &tx_disable) < 0)
    {
        return scnprintf(buf, BUF_SIZE, "%d\n", err_code);
    }
                     ...
                     char *buf)
{
    ...
    int port = 0;
    int port_num = port_num_get();
    if (Sff.drv->prsL_all_get(&bitmap) < 0)
    {
        //return scnprintf(buf, BUF_SIZE, "%d\n", err_code);
        return err_code;
    }
...
static struct sff_attribute sff_prsL_all_attr =
    __ATTR(prsL_all, S_IRUGO, sff_prsL_all_show, NULL);
...
static int sff_obj_destroy(void)
{
...
int sff_driver_register(struct sff_driver_t *drv)
{
...
EXPORT_SYMBOL(sff_driver_register);

int sff_driver_unregister(void)
{
...
EXPORT_SYMBOL(sff_driver_unregister);
/*
 sff init sequence
...
*/
#if 0
static struct port_info_map_t *_find_port_info(int port, struct port_info_map_t map[], int obj_num)
{
    int id = 0;
...
#endif
static int sff_obj_init(struct port_info_table_t *tbl)
{
...
static int sff_obj_create(int size)
{
...
static int sff_fsm_kobj_change_event(int port)
{

    struct sff_kobj_t *sff_kobj = Sff.obj[port].kobj; 
    char *uevent_envp[4];
    char tmp_str_1[32];
...
static int sff_fsm_kobj_add(int port) 
{
    struct sff_obj_t *sff_obj = &Sff.obj[port]; 
    ...
    sff_obj->kobj = sff_kobj_add(sff_obj->name, &Sff_Kset->kobj, attr_group);
...
static int sff_fsm_kobj_del(int port) 
{
    struct sff_kobj_t **sff_kobj = &(Sff.obj[port].kobj); 
...
static int sff_kobj_init_create(void)
{
    ...
    Sff.common_kobj = sff_kobj_add("common", &Sff_Kset->kobj, &common_group); 
    if(IS_ERR_OR_NULL(Sff.common_kobj)) {

        goto exit_kfree_kset;
    }
...
static void sff_kobj_destroy(int size)
{
...
static void transvr_insert(int port)
{
    int old_st = sff_fsm_st_get(port);
    SFF_MGR_DEBUG("transvr:%d insert\n", port);
    sff_fsm_st_set(port, SFF_FSM_ST_INSERTED);
    sff_fsm_state_change_process(port, old_st, sff_fsm_st_get(port));    
    transvr_type_set(port, TRANSVR_CLASS_UNKNOWN);    
    sff_fsm_kobj_add(port);
}
static void transvr_remove(int port)
{
    int old_st = sff_fsm_st_get(port);
    SFF_MGR_DEBUG("transvr:%d remove\n", port);
    sff_fsm_st_set(port, SFF_FSM_ST_REMOVED);        
    sff_fsm_state_change_process(port, old_st, sff_fsm_st_get(port));    
    transvr_type_set(port, TRANSVR_CLASS_UNKNOWN);    
    sff_fsm_kobj_del(port);
}
...
static int prsL_scan(void)
{
    ...
    * when other app access broken transciver eeprom*/
    ...
    prsL_change = bitmap ^ prsL_bitmap_get(); 
    prsL_bitmap_update(bitmap); //update current prsL_bitmap 
...
static void sff_fsm_delay_cnt_reset(int port, sff_fsm_state_t st)
{
   int found = 0;
   int idx = 0;
   struct fsm_period_t *table = Sff_Fsm[port].period_tbl;
   ...
   if (found) {
      Sff_Fsm[port].delay_cnt = table[idx].delay_cnt;
   } else {
      Sff_Fsm[port].delay_cnt = 0; 
   } 
}    
static void sff_fsm_cnt_run(int port)
{
...
{
    bool is_hit = false;
    if(Sff_Fsm[port].cnt >= Sff_Fsm[port].delay_cnt)
    {  
         Sff_Fsm[port].cnt = 0; 
         is_hit = true;
    }   
...
static bool sfp_eth_comp_is_supported(int port, u8 eth_comp)
{
    ...
    if(idx >= size)
    {
      ...
      SFF_MGR_DEBUG("port:%d unknown eth_cmp:0x%x\n", port, (eth_comp & 0xf0) >> 4);
    }
    
    switch (comp_codes)
    {
        case SR_10GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_10G_S_SR);
            break;
        case LR_10GBASE:
        case LRM_10GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_10G_S_LR);
            break;
        case ER_10GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_10G_S_ER);
            break;
        default:
        break;

    }    
    if (TRANSVR_CLASS_UNKNOWN != transvr_type_get(port))
    {
        is_supported = true;
        SFF_MGR_DEBUG("port:%d known eth_ext_cmp:0x%x transvr:%d\n", 
                      port, eth_comp, transvr_type_get(port));
    }    
    return is_supported;
}    
static bool qsfp_eth_comp_is_supported(int port, u8 eth_comp)
{
    bool is_supported = false;
    ...
    for (idx = 0; idx < size; idx++)
    {
        if(eth_comp & Qsfp_Eth_Comp_Table[idx])
        {
            ...
            SFF_MGR_DEBUG("port:%d known eth_cmp:0x%x\n", port, eth_comp & Qsfp_Eth_Comp_Table[idx]);
            break;
        }    
    }    
    if(idx >= size)
    ...
    {
        case ACTIVE_CABLE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_40G_AOC);
            break;
        case LR4_40GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_40G_LR4);
            break;
        case SR4_40GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_40G_SR4);
            break;
        case CR4_40GBASE:
            transvr_type_set(port, TRANSVR_CLASS_COPPER_L4_40G);
            break;
        default:
            break;
    }    
...
static bool sfp_eth_ext_is_supported(int port, u8 eth_ext_comp)
{
    bool is_supported = false;
    ...
    switch(eth_ext_code)
    {
        ...
        case AOC_100G_BER_12: /* 100G AOC or 25G AUI C2M AOC 10^^-12 BER */
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_25G_AOC);
            break;
        case SR4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_25G_SR);
            break;
        case LR4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_25G_LR);
            break;
        case ER4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_25G_ER);
            break;
        case CR4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_COPPER_L1_25G);    
            break;
        case ACC_100G_BER_5:
    ...
    if (TRANSVR_CLASS_UNKNOWN != transvr_type_get(port))
    {
        ...
        SFF_MGR_DEBUG("port:%d known eth_ext_cmp:0x%x transvr_type:%d\n", 
                      port, eth_ext_comp, transvr_type_get(port));
    }    
    return is_supported;
}  
static bool qsfp_eth_ext_is_supported(int port, u8 eth_ext_comp)
{
    bool is_supported = false;
    Ethernet_extended_compliance eth_ext_code = eth_ext_comp;
    ...
    switch(eth_ext_code)
    {
        ...
        case AOC_100G_BER_12:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_100G_AOC);
            break;
        case SR4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_100G_SR4);
            break;
        case LR4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_100G_LR4);
            break;
        case ER4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_100G_ER4);
            break;
        case PSM4_100G_SMF:
            transvr_type_set(port, TRANSVR_CLASS_OPTICAL_100G_PSM4);
            break;
        case CR4_100GBASE:
            transvr_type_set(port, TRANSVR_CLASS_COPPER_L4_100G);
            break;
        case SR10_100GBASE:
        case CWDM4_100G:
        case ACC_100G_BER_5:
...
static int sfp_type_identify(int port, u8 *is_found)
{
    u8 id = 0;
    u8 transvr_codes[8];
    u8 conn_type = 0;
    ...
    if((ret = _sfp_eth_extended_compliance_get(port, &eth_ext_comp)) < 0)
    {
        goto exit_err;
    }
    
    if(!sfp_eth_ext_is_supported(port, eth_ext_comp))
    {
  
        if((ret = _sfp_transvr_codes_read(port, transvr_codes, ARRAY_SIZE(transvr_codes))) < 0)
        {
            goto exit_err;
        }    
        ...
        for (idx = 0; idx < 8; idx++)
        {
            SFF_MGR_DEBUG("port:%d transvr[%d]:0x%x\n", port, idx, transvr_codes[idx]);    
        }    
        
        if((ret = _sfp_connector_type_read(port, &conn_type)) < 0)
        {
            goto exit_err;
        }    
        ...
        if (0x07 == conn_type)
        {    
            if(!sfp_eth_comp_is_supported(port, transvr_codes[0]))
            {
...
/* */
static int qsfp_type_identify(int port, u8 *is_found)
{
    u8 id = 0;
    u8 eth_comp = 0;
    u8 eth_ext_comp = 0;
    int ret = 0; 
    if((ret = _qsfp_id_read(port, &id)) < 0)
    {
        goto exit_err;  
    }   
    if(0x0d != id && 
    ...
    if((ret = _qsfp_ethernet_compliance_read(port, &eth_comp)) < 0)
    {
        goto exit_err;
    }    
    ...
    {
       if((ret = _qsfp_eth_extended_compliance_get(port, &eth_ext_comp)) < 0)
       {
            goto exit_err;
       }    
        
        if(!qsfp_eth_ext_is_supported(port, eth_ext_comp))
        {
            goto exit_non_support_err;
        }    
    }
    else /*40G*/
    {
        ...
        */
        ...
        if(!qsfp_eth_comp_is_supported(port, eth_comp))
        {
            goto exit_non_support_err;
        }
    }
...
static int sfp_channel_control_set(int port, int type, u32 value)
{
    ...
    if( sfp_eeprom_write(port, slave_addr, offset+index, &data, sizeof(data)) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
...
static int sfp_channel_control_get(int port, int type, u32 *value)
{
    ...
    if( sfp_eeprom_read(port, slave_addr, offset+index, &data, sizeof(data)) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
...
static int qsfp_channel_control_set(int port, int type, u32 value)
{
    ...
    if( qsfp_eeprom_write(port, page, offset, ch_ctrl, len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
...
static int qsfp_channel_control_get(int port, int type, u32 *value)
{
    ...
    if( qsfp_eeprom_read(port, page, offset, opt_indicator, len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }    
    ...
    if( qsfp_eeprom_read(port, page, offset, ch_ctrl, len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
...
static int qsfp_dd_force_low_pwr(int port, int en)
{
    u8 buf = 0; 
    int ret = 0;
    u8 page = QSFP_PAGE0_LOWER;
    u8 offset = 26;
    if ((ret = qsfp_eeprom_read(port, page, offset, &buf, 1)) < 0) {
    ...
    if (en) {
     ...
     if( (ret = qsfp_eeprom_write(port, page, offset, &buf, 1)) < 0)
     {
        SFF_MGR_ERR("write fail\n");
        return ret;
     }
...
static int qsfp_dd_active_control_set_indicator(int port)
{
    ...
    if (!is_bank_num_valid(port)) {
        return -1;
    }
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE_11h, 206, data, size)) < 0) {
...
static int qsfp_dd_channel_control_get(int port, int type, u32 *value)
{
    ...
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE_01h, 161, data, 2)) < 0) {
...
int sfp_channel_status_get(int port, int type, u8 *value)
{
    ...
    switch(type)
    {
        case CH_STATUS_RX_LOS_TYPE:
            {    
                ret = Sff.drv->rx_los_get(port, &pin); 
            }
            break;
        case CH_STATUS_TX_FAULT_TYPE:
            {    
                ret = Sff.drv->tx_fault_get(port, &pin); 
            }
            break;
        default:
            goto exit_err;
...
int qsfp_lane_status_get(int port, int type, u8 *st)
{
    *st = Sff.obj[port].private_data.qsfp.lane_st[type];
    return 0;
}    
...
int qsfp_lane_status_update(int port, int type, u8 value)
{
    ...
    Sff.obj[port].private_data.qsfp.lane_st[type] = value;
    
    return 0;

}    
int qsfp_channel_status_get(int port, int type, u8 *value)
{
    ...
    if( qsfp_eeprom_read(port, page, offset, (u8 *)(&interrupt_flag), len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }    
...
static int qsfp_temperature_get(int port, u8 *buf, int buf_size)
{
    ...
    if( qsfp_eeprom_read(port, page, offset, byte_data, len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
...
static int qsfp_channel_monitor_get(int port, int type, u8 *buf, int buf_size)
{
    ...
    if( qsfp_eeprom_read(port, page, offset, ch_monitor, len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
...
static int qsfp_dd_temperature_get(int port, u8 *buf, int buf_size)
{
    ...
    if( qsfp_eeprom_read(port, page, offset, byte_data, WORD_SIZE) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
...
static int qsfp_cdr_control(int port)
{
    ...
    if( qsfp_eeprom_read(port, page, offset, &ext_id, len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }    
    ...
    if (ext_id & 0x0c) //cdr tx/rx are prsL , enable cdr control
    {
        ...
        /*debugging purpose*/
         ...
         if(qsfp_eeprom_read(port, page, offset, &cdr_ctrl, len) < 0)
         {
            goto exit_err;
         }    
         ...
         {    
             cdr_ctrl = 0xff;   
             if( qsfp_eeprom_write(port, page, offset, &cdr_ctrl, len) < 0)
             {
...
static int sfp_tx_disable_set(int port, u8 value)
{
    ...
    u8 status = 0;
    int rec = 0;
    if (IS_ERR_OR_NULL(Sff.drv->tx_disable_set)) {

        goto exit_err;    
    }   
     
    rec = Sff.drv->tx_disable_set(port, value);
    if (rec < 0) {
        goto exit_err;
    }    
    ...
    if (REC_SFF_IO_UNSUPPORTED == rec) {  
        ...
        if( sfp_eeprom_read(port, slave_addr , offset, &status, len) < 0)
        {
          goto exit_err;   
        ...
        if( sfp_eeprom_write(port, slave_addr , offset, &status, len) < 0)
        {
            goto exit_err;
        }
...
static int sfp_tx_disable_get(int port, u8 *value)
{
    ...
    if (IS_ERR_OR_NULL(Sff.drv->tx_disable_get)) {
    ...
    rec = Sff.drv->tx_disable_get(port, value);
...
static int qsfp_tx_disable_set(int port, u8 value)
{
    ...
    if( qsfp_eeprom_read(port, page, offset, &tx_disable, len) < 0)
    {
        SFF_MGR_ERR("NG1\n");
        goto exit_err;
    }
    ...
    if( qsfp_eeprom_write(port, page, offset, &value, len) < 0)
    {
        SFF_MGR_ERR("NG2\n");
        goto exit_err;
    }
...
static int tx_disable_set(int port, u8 value)
{
    if (SFP_TYPE == Sff.obj[port].type) {
        return sfp_tx_disable_set(port, value);
    } else if (QSFP_TYPE == Sff.obj[port].type) {
       
        return qsfp_tx_disable_set(port, value);
 
    } else { /*reserve for qsfp_dd*/

        return qsfp_dd_tx_disable_set(port, value);
    }
    return 0;    
}    
...
{
    if (SFP_TYPE == Sff.obj[port].type) {
        return sfp_tx_disable_get(port, value);
    } else if (QSFP_TYPE == Sff.obj[port].type) {
        return qsfp_tx_disable_get(port, value);
    } else { /*reserve for qsfp_dd*/

        return qsfp_dd_tx_disable_get(port, value);
    }
    return 0;    

}    
static int sfp_rate_select(int port, u8 rate_bitmap)
{
    ...
    {    
        ...
        if( sfp_eeprom_read(port, slave_addr , offset, &ext_status, len) < 0)
        {
          goto exit_err;   
        ...
        if( sfp_eeprom_write(port, slave_addr , offset, &ext_status, len) < 0)
        {
            goto exit_err;
        }
        #ifdef READBACK_CHECK
...
static int sfp_rate_select_control(int port)
{
    ...
    if( sfp_eeprom_read(port, slave_addr , offset, &rate_id, len) < 0)
    {
        goto exit_err;
    }    
    ...
    switch (rate_id) {
        ...
        if(sfp_rate_select(port, SOFT_RX_RATE_RS0) < 0)
        {
            goto exit_err;
        }
        found = true;    
        ...
        if(sfp_rate_select(port, SOFT_TX_RATE_RS1) < 0)
        {
            goto exit_err;
        }
        found = true;    
        ...
        if(sfp_rate_select(port, SOFT_RX_RATE_RS0|SOFT_TX_RATE_RS1) < 0)
        {
            goto exit_err;
        }
        found = true;    
...
static int qsfp_intL_get(int port, u8 *value)
{
 ...
 return Sff.drv->intL_get(port, value);   

}    
static int lpmode_set(int port, u8 value)
{
 
 return Sff.drv->lpmode_set(port, value);   

}    
static int lpmode_get(int port, u8 *value)
{
 
 return Sff.drv->lpmode_get(port, value);   

}    
static int mode_sel_set(int port, u8 value)
{
 
 return Sff.drv->mode_sel_set(port, value);   

}    
static int mode_sel_get(int port, u8 *value)
{
 
 return Sff.drv->mode_sel_get(port, value);   
...
static void sff_fsm_state_change_process(int port, 
                                        ...
                                        sff_fsm_state_t next_st)
{
    ...
    if (cur_st != next_st)
    {
        ...
        sff_fsm_delay_cnt_reset(port, next_st); 
        //SFF_MGR_DEBUG("port:%d st change:%d -> %d\n", 
          //            port, st,sff_fsm_st_get(port)); 
        SFF_MGR_DEBUG("port:%d st change:%s -> %s\n", 
                      port, sff_fsm_st_str[cur_st], 
...
static int sff_fsm_sfp_task(int port)
{
    sff_fsm_state_t st = sff_fsm_st_get(port);
    int ret = 0;
    switch (st) {
    ...
    case SFF_FSM_ST_REMOVED:
        sff_fsm_st_set(port, SFF_FSM_ST_IDLE);        
        break;
    case SFF_FSM_ST_INSERTED:
        {
            u8 is_ready = 0;
            ret = _sfp_data_ready_check(port, &is_ready);
            if (ret < 0)  
            {
                break;
            }
            ...
            if(is_ready) {
                sff_fsm_st_set(port, SFF_FSM_ST_DETECTING);
            }  
        }
        break;
    ...
    case SFF_FSM_ST_DETECTING:
        {
            u8 is_found = 0;
            if((ret = sfp_tx_disable_set(port, TX_1CH_DISABLE_ON)) < 0)
            {
                break;
            }
            if ((ret = sfp_rate_select_control(port)) < 0) {
                break;
            }
            if((ret = sfp_type_identify(port, &is_found)) < 0)
            {    
                break;
            }
            ...
            if(is_found)
            {
                sff_fsm_st_set(port, SFF_FSM_ST_INIT);
            }
            else
            {
              /*unknown type: special handling*/
                
                sff_fsm_st_set(port, SFF_FSM_ST_UNKNOWN_TYPE);
            }    
    ...
    case SFF_FSM_ST_INIT:
        if((ret = sfp_tx_disable_set(port, TX_ALL_CH_DISABLE_OFF)) < 0)
        {
            break;
        }
        sff_fsm_st_set(port, SFF_FSM_ST_DETECTED);
        break;
    case SFF_FSM_ST_FAULT:
        break;    
    ...
    sff_fsm_state_change_process(port, st, sff_fsm_st_get(port));
    return 0;
}   
#if 0 /*<TBD>*/
...
static int qsfp_int_flag_read(int port)
{
    ...
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 0x03, &buf, 1)) < 0) {
    ...
    qsfp_lane_status_update(port, CH_STATUS_RX_LOS_TYPE, data);
    //if (data) {
        SFF_MGR_DEBUG("port:%d rxlos 0x%x", port, data);
    //}
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 0x04, &buf, 1)) < 0) {
    ...
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 0x05, reg, 17)) < 0) {
...
static int sff_fsm_qsfp_task(int port)
{
    ...
    case SFF_FSM_ST_INSERTED:
        {
            u8 is_ready = 0;
            ret = _qsfp_data_ready_check(port, &is_ready);
            if (ret < 0)  
            {
                break;
            }
    ...
    case SFF_FSM_ST_DETECTING:
        {
            u8 is_found = 0;
            if ((ret = qsfp_tx_disable_set(port, TX_4CH_DISABLE_ON)) < 0) {
            
                break;
            }
            if((ret = qsfp_type_identify(port, &is_found)) < 0)
            {    
                break;
            }
    ...
    case SFF_FSM_ST_INIT:
        {
            /*cdr control*/
            if((ret = qsfp_cdr_control(port)) < 0)
            {
                break;
            }

            /*low power mode control*/
            if((ret = qsfp_lpmode_control(port)) < 0)
            {
                break;
           ...
           /*tx_disable control*/
            ...
            if((ret = qsfp_tx_disable_set(port, TX_ALL_CH_DISABLE_OFF)) < 0)
            {
                break;
            }
            sff_fsm_kobj_change_event(port);    
            sff_fsm_st_set(port, SFF_FSM_ST_DETECTED);
        }
        break;
...
static bool transvr_is_i2c_ready(void)
{
    ...
    for (port = 0; port < port_num; port++) { 
        buf = 0;
        if (SFF_FSM_ST_ISOLATED != sff_fsm_st_get(port)) {
            
            ret = Sff.drv->eeprom_read(port, addr, 0, &buf, 1);         
            if (ret < 0) {
               if (!ioexp_is_channel_ready()){
...
static int eeprom_read_retry(int port, u8 addr, u8 offset, u8 *buf, int len)
{
    int try = 0;
    int ret = 0;
    for (try = 0; try < 1; try++) {
        ret = Sff.drv->eeprom_read(port, addr, offset, buf, len);         
...
static void _bad_transvr_detect_phase0(void)
{
    ...
    * so read the data of that channel after mux is reset will fail , so need to switch to other channels first */ 
    ...
    for (port = 1; port < port_num; port+=8) { 
        ...
        if (SFF_FSM_ST_ISOLATED != sff_fsm_st_get(port)) 
        {
            ret = eeprom_read_retry(port, addr, 0, &buf, 1);         
            ...
            if (ret >= 0) {
                ...
                if (!ioexp_is_channel_ready())
                {
                    //msleep(100);
                    io_mux_reset_all_seq();
                    if (SFF_FSM_ST_IDLE != sff_fsm_st_get(port)) {      
                        old_st = sff_fsm_st_get(port);
                        SFF_MGR_ERR("transvr[%d]:isolated\n", port);
                        sff_fsm_st_set(port, SFF_FSM_ST_ISOLATED);
                        sff_fsm_state_change_process(port, old_st, sff_fsm_st_get(port));
                    }
                    break;
                } 
...
static int sff_fsm_run(void)
{
    ...
    for(port = 0; port < port_num; port++)
    {
        if (sff_fsm_delay_cnt_is_hit(port))
        {    
            ret = Sff_Fsm[port].task(port); 
            if (ret < 0) {
                return ret;
            }   
        }
        sff_fsm_cnt_run(port);
    }    
    return 0;
}
...
static int qsfp_dd_module_st_get(int port, u8 *st)
{
    u8 data = 0;
    int ret = 0;
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 0x03, &data, 1)) < 0) {
    ...
    *st = masked_bits_get(data, 1, 3);
    ...
    if (qsfp_dd_id_status_get(port, &data) < 0)
    {
        return -1;
    }
...
static int qsfp_dd_data_path_st_get(int port, u32 *st)
{
    u32 data = 0;   
    ...
    if(qsfp_eeprom_read(port, QSFP_PAGE_11h, 128, (u8*)&data, 4) < 0)
    {
        SFF_MGR_ERR("qsfp_eeprom_read fail");
        *st = 0; 
        return -1;
...
static bool qsfp_dd_is_data_path_activated(int port)
{
    u32 data_path_st;
    u32 st_lane = 0;
    int ln = 0;
    bool is_active = false; 
    int lane_num = valid_lane_num_get(port); 
    
    if (qsfp_dd_data_path_st_get(port, &data_path_st) < 0) {
        return is_active;
    }    
...
static int qsfp_dd_id_get(int port, u8 *val)
{
    ...
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 0, &buf, 1)) < 0) {
...
static int qsfp_dd_id_status_get(int port, struct qsfp_dd_id_status_t *data)
{
    ...
    if (qsfp_eeprom_read(port, page, offset, (u8 *)(&tmp), len) < 0) {
        return -1;
    }
    memcpy(data, &tmp, len);
    return 0;
 ...
 */
static int qsfp_dd_module_state_is_changed(int port, bool *is_changed)
{
    ...
    u8 val = 0;
    int ret = 0;
    if ((ret = qsfp_eeprom_read(port, page, offset, &val, 1)) < 0) {
    ...
    *is_changed = ((val&0x1) ? true : false);
    ...
    if (qsfp_eeprom_read(port, page, offset, (u8 *)(&flags), len) < 0)
    {
        return -1;
    }
...
static int qsfp_dd_digital_int_get(int port, u8 *value)
{
    u8 st = 0;
    int ret = 0;
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 0x03, &st, 1)) < 0) {
...
#endif
static int qsfp_dd_intL_get(int port, u8 *value)
{
    return Sff.drv->intL_get(port, value);
}   
static bool is_bank_num_valid(int port)
{
    ...
    bool valid = false;
    /*check if it's bank0? */
    if (qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 126, &bank_no, 1) < 0) {
    ...
    if (0x00 == bank_no) {
        ...
        SFF_MGR_ERR("port:%d bank no is not 0", port);
    } 
...
static int qsfp_dd_media_interface_tech_get(int port, u8 *interface)
{
    ...
    if(qsfp_eeprom_read(port, QSFP_PAGE0_UPPER, 212, &data, 1) < 0)
    {
        SFF_MGR_ERR("qsfp_eeprom_read fail");
...
#endif
static bool is_loopback_module(int port) 
{
    /*<TBD> need check more loopback modules*/
    if (0 == Sff.obj[port].private_data.qsfp_dd.module_type) {
...
static int app_advertising_field_get(int port)
{
    u8 module_type = 0;
    union qsfp_dd_app_advert_fields *fields = Sff.obj[port].private_data.qsfp_dd.fields;
    int ret = 0;
    int apsel = 0;
    int i = 0;
    bool end_of_table = false;
    
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 85, &module_type, 1)) < 0) {

        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return ret;
    }
    
    SFF_MGR_INFO("port:%d module_type:0x%x\n", port, module_type);
    Sff.obj[port].private_data.qsfp_dd.module_type = module_type;
    for (apsel = APSEL_1; apsel < APSEL_NUM; apsel++) {
       for (i = 0; i < APP_ADVERT_FIELD_NUM; i++) { 
       
           if ((ret =qsfp_eeprom_read(port, app_advert_fields_page[i], 
    ...
    for (apsel = 0; apsel < APSEL_NUM; apsel++) {
        SFF_MGR_INFO("port:%d host_electrical_interface_code[%d]:0x%x\n", 
                     port, apsel, fields[apsel].data.host_electrical_interface_code);
        SFF_MGR_INFO("port:%d module_media_interface_code[%d]:0x%x\n", 
                    port, apsel, fields[apsel].data.module_media_interface_code);
        SFF_MGR_INFO("port:%d lane_count[%d]:0x%x\n", 
                    port, apsel, fields[apsel].data.lane_count);
        SFF_MGR_INFO("port:%d  host_lane_assignment_options[%d]:0x%x\n", 
...
static int app_advert_field_dump(int port, u8 *buf, int buf_size)
{
    ...
    ret = app_advertising_field_get(port);
    if (ret < 0) {
        return ret;
    }
    scnprintf(buf, buf_size,
              ...
              Sff.obj[port].private_data.qsfp_dd.module_type,
...
static int stage_control_set0(int port)
{
    u8 apply_stage_control_set = 0;
    u8 stage_set_offset_begin = 145;
    u8 stage_set_offset_end = stage_set_offset_begin + valid_lane_num_get(port) - 1;
    u8 offset = 0;
    ...
    for (offset = stage_set_offset_begin; offset <= stage_set_offset_end; offset++) {
        if(qsfp_eeprom_write(port, QSFP_PAGE_10h, offset, (u8*)(&stage_set), 1) < 0) {
            return -1;
        }
    }
    /*table 55 Apply_DataPathInit*/  
    apply_stage_control_set = 0xff;
    if(qsfp_eeprom_write(port, QSFP_PAGE_10h, 143, &apply_stage_control_set, 1) < 0)
    {
        return -1;
    }
    return 0;
 ...
 * port(in) 
 * is_pass(out)*/
static int config_error_code_check(int port, bool *is_pass)
{
    u8 data[4];
    int lane = 0;
    int i = 0;
    ...
    *is_pass = false; 
    ...
    if (qsfp_eeprom_read(port, QSFP_PAGE_11h, 202, data, 4) < 0) {
        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return -1;
    }
    lane = 0;
...
static int qsfp_dd_tx_disable_set(int port, u8 val)
{
    ...
    if(qsfp_eeprom_write(port, QSFP_PAGE_10h, 130, &val, 1) < 0)
    {
        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return -1;
    }
...
static int qsfp_dd_tx_disable_get(int port, u8 *val)
{
    ...
    if(qsfp_eeprom_read(port, QSFP_PAGE_10h, 130, &buf, 1) < 0)
    {
        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return -1;
    }
...
/*table 53 */
static int data_path_power_up(int port, bool up)
{
    ...
    if(qsfp_eeprom_write(port, QSFP_PAGE_10h, 128, &value, 1) < 0)
    {
        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return -1;
    }
...
static int qsfp_dd_is_flat_mem(int port, bool *is_flat)
{
    u8 buf = 0;
    u8 offset = 2;
    ...
    if (qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, offset, &buf, 1) < 0) {
...
static bool is_copper(int port)
{
    ...
    if (qsfp_dd_is_flat_mem(port, &is_flat) < 0)
    {
       return is_copper; 
    }
    if (is_flat) {
...
static int qsfp_dd_interrupt_flags_read(int port)
{
    ...
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 8, module_flags, 4)) < 0) {

        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return ret;
    }
    /* table 16 Lane-Specific Flag*/ 
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE_11h, 134, lane_flags, 19)) < 0) {
...
/*table 15*/
static int qsfp_dd_module_flag_monitor(int port)
{

    int ret = 0;
    int i = 0;
    u8 module_flag[4];
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE0_LOWER, 8, module_flag, 4)) < 0) {
...
static int qsfp_dd_lane_flag_monitor(int port)
{
    ...
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE_11h, 134, &data_path_st_change, 1)) < 0) {

        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return ret;
    }
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE_11h, 135, tx_flag, 12)) < 0) {

        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return ret;
    }
    
    if ((ret = qsfp_eeprom_read(port, QSFP_PAGE_11h, 147, rx_flag, 6)) < 0) {

        SFF_MGR_ERR("port:%d qsfp_eeprom_read fail", port);
        return ret;
    }
    SFF_MGR_INFO("port:%d  data path st change:0x%x\n", port, data_path_st_change);
    for (i = 0; i < 12; i++) {
...
static int sff_fsm_qsfp_dd_task(int port)
{
    sff_fsm_state_t st = sff_fsm_st_get(port);
    int ret = 0;
    switch (st) {
    ...
    case SFF_FSM_ST_INSERTED:
        {
            sff_fsm_st_set(port, SFF_FSM_ST_MGMT_INIT);
        }    
        break;
    ...
    case SFF_FSM_ST_MGMT_INIT:
        {
            ...
            *check if Software Init mode 
...
#if 0 
            if (qsfp_dd_intL_get(port, &intL) < 0)
            {
                break;
            }    
            if (intL) {
            ...
            if (qsfp_dd_module_state_is_changed(port, &is_changed) < 0) {
                break;
            }
            ...
            if ((ret = qsfp_dd_module_st_get(port, &value)) < 0) {
            ...
            if (is_copper(port)) {
                ...
                if ((ret = lpmode_get(port, &lpmode)) < 0) {
                    break;
                }
                if (lpmode) {
                    ...
                    {
                        sff_fsm_st_set(port, SFF_FSM_ST_MODULE_LOW_PWR_INIT);
                    }   
                } else {
                    sff_fsm_st_set(port, SFF_FSM_ST_MODULE_HW_INIT);   
                }       
            }
        }    
        break;
    case SFF_FSM_ST_MODULE_LOW_PWR_INIT:
        {
            ...
            */   
            ...
            ret = qsfp_dd_active_control_set_indicator(port);
            if (ret < 0) {
                break;
            }
            if ((ret = app_advertising_field_get(port)) < 0) {
                /*print error code*/
                break;
            }
            if (is_loopback_module(port)) {

                if ((ret = data_path_power_up(port, true)) < 0) {
                    break;
                }       
                sff_fsm_st_set(port, SFF_FSM_ST_MODULE_LOOPBACK_INIT);   
            } else {
                /* host select application*/
                /*stage control set 0*/
                if ((ret = stage_control_set0(port)) < 0) {
                    /*print error code*/
                    break;
                }
                sff_fsm_st_set(port, SFF_FSM_ST_MODULE_LOW_PWR_CONFIG_CHECK);  
            } 
        }    
        break;
    case SFF_FSM_ST_MODULE_LOW_PWR_CONFIG_CHECK:
        {
            bool is_pass = false;
            if ((ret = config_error_code_check(port, &is_pass)) < 0) {
            ...
            if (is_pass) {
                ...
                sff_fsm_st_set(port, SFF_FSM_ST_MODULE_LOW_PWR_CONTROL);   
            }
        }    
        break;
    case SFF_FSM_ST_MODULE_LOW_PWR_CONTROL:
        {
            ...
            if ((ret = qsfp_dd_tx_disable_set(port, TX_8CH_DISABLE_ON)) < 0)
            {
                break;
            }    
             /*Host sets the 
            ...
            sff_fsm_st_set(port, SFF_FSM_ST_MODULE_WAITING_READY);   
        }    
        break;
    ...
    case SFF_FSM_ST_MODULE_LOOPBACK_INIT:
        {
            ...
            if ((ret = qsfp_dd_module_st_get(port, &module_st)) < 0) {
                break;
            }
    ...
    case SFF_FSM_ST_MODULE_WAITING_READY:
        {
            ...
            if (qsfp_dd_digital_int_get(port ,&intL) < 0) {
            ...
            if(is_changed) {    
                /*print log*/    
                SFF_MGR_INFO("module st change st:%d\n", module_st);
            }    
#endif
            /*check module state register if it transit to module ready*/
            if (MODULE_READY_ST_ENCODE == module_st && 
                qsfp_dd_is_data_path_activated(port)) {
                sff_fsm_st_set(port, SFF_FSM_ST_MODULE_READY);        
            }    
        }    
        break;
    case SFF_FSM_ST_MODULE_READY:
        {
            ...
            if ((ret = qsfp_dd_tx_disable_set(port, TX_ALL_CH_DISABLE_OFF)) < 0)
            {
                break;
            }    
            sff_fsm_st_set(port, SFF_FSM_ST_DETECTED);        
    ...
    case SFF_FSM_ST_DETECTED:
        { 
            ...
            if (int_flag_monitor_en) {
                if (!is_copper(port)) { 
                    if ((ret = qsfp_dd_intL_get(port, &intL)) < 0)
                    {
                        break;
                    }    
                    if (!intL) {
                        if ((ret = qsfp_dd_module_flag_monitor(port)) < 0) {
                            break;
                        }
                        if ((ret = qsfp_dd_lane_flag_monitor(port)) < 0 ) {
                            break;
                        }
                    }
                }
...
static int sff_fsm_create(int size)
{
    ...
    Sff_Fsm = kzalloc(sizeof(struct sff_fsm_t) *size, GFP_KERNEL);
    
    if (IS_ERR_OR_NULL(Sff_Fsm))
    {
        SFF_MGR_ERR("NULL ptr\n");
        return -ENOMEM;
    }
...
static int sff_fsm_init(struct port_info_table_t *tbl)
{
    int port = 0;
    int port_num = 0;
    struct port_info_map_t *map = NULL;
    ...
    for (port = 0; port < port_num; port++)
    {
        ...
        if(SFP_TYPE == type) {  
           Sff_Fsm[port].task = sff_fsm_sfp_task;
           Sff_Fsm[port].period_tbl = fsm_period_tbl;
        
        } else if (QSFP_TYPE == type) {
           
           Sff_Fsm[port].task = sff_fsm_qsfp_task;
           Sff_Fsm[port].period_tbl = fsm_period_tbl;
        
        } else if (QSFP_DD_TYPE == type) {

           Sff_Fsm[port].task = sff_fsm_qsfp_dd_task;
           Sff_Fsm[port].period_tbl = fsm_period_tbl;

        }
         Sff_Fsm[port].st = SFF_FSM_ST_IDLE;
         Sff_Fsm[port].cnt = 0;
         Sff_Fsm[port].delay_cnt = 0;
 ...
 *
 * */
static int sff_module_init_seq(void)
{
    int port = 0;
    int port_num = port_num_get();
    int type = 0;
    ...
    for (port = 0; port < port_num; port++) {   
        ...
        if ((ret = prsL_get(port, &prsL)) < 0) {
            return ret;
        }
        if (!prsL) { 
            /*assert resetL*/   
            if ((ret = Sff.drv->reset_set(port, 0)) < 0) {
                return ret;
            }
            msleep(RESET_ASSERT_TIME);
            /*de-assert resetL*/   
            if ((ret = Sff.drv->reset_set(port, 1)) < 0) {
                return ret;
            }
...
static void sff_data_init(void)
{
...
static int __init sff_init(void)
{
    ...
    if(sff_obj_create(Port_Info_Tbl->size) < 0)
    {
        SFF_MGR_ERR("sff_obj_create fail\n");
        goto exit_err;
    }

    if(sff_kobj_init_create() < 0)
    {
        SFF_MGR_ERR("sff_kobj_init_create fail\n");
        goto exit_kfree_sff_obj;

    }
    if(sff_fsm_create(Port_Info_Tbl->size) < 0)
    {

        SFF_MGR_ERR("sff_fsm_create_all fail\n");
        goto exit_kfree_sff_kobj;
    }
    
    if(sff_obj_init(Port_Info_Tbl) < 0)
    {
        SFF_MGR_ERR("sff_obj_init fail\n");
        goto exit_kfree_sff_fsm;
    }
    
    if(sff_fsm_init(Port_Info_Tbl) < 0)
    {
        SFF_MGR_ERR("sff_fsm_init fail\n");
        goto exit_kfree_sff_fsm;
    }
      
    /*init sff mgr parameters*/
    sff_data_init();     
    if (sff_driver_register(&Sff_Drv) < 0) {
    ...
    if (sff_module_init_seq() < 0 ) {

        SFF_MGR_ERR("sff_module_init_seq fail\n");
        goto exit_kfree_sff_fsm;
    }
    ...
    sff_kobj_destroy(Port_Info_Tbl->size);

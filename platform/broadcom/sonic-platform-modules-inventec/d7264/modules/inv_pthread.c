 ...
 * LED definitions
 ************************/
#define SINGLE_STATUS_LED       0       // 1:true  0:false(4 status leds)
...
#define STATUS_LED_INVALID      -1      // Invalid

// led code for psu, fan, beacon
#define STATUS_LED_OFF          ("0")   // 0 -- 00: off
#define STATUS_LED_GRN          ("1")   // 1 -- 01: green
#define STATUS_LED_RED          ("2")   // 2 -- 10: red
#define STATUS_LED_AMBER        ("3")   // 3 -- 11: amber

#define STATUS_SYS_LED          ("sys_led")
#define STATUS_FAN_LED          ("fan_led")
#define STATUS_PSU_LED          ("psu_led")
#define STATUS_BEACON_LED       ("beacon_led")

#if SINGLE_STATUS_LED
...
#else
static int sys_led_grn = 1;
static int fan_led_grn = 1;
static int psu_led_grn = 1;
ssize_t set_status_led(const char *color, const char *led_type);
int status_led_check_color(const char *led_type);
#endif
...
int sysfs_detect_hwmon_index(void)
{
    ...
    for (hwid = 0; hwid < INV_HWMID_MAX && (hwm_psoc == INV_HWMID_INIT || hwm_cpld == INV_HWMID_INIT); hwid++) {
        ...
        else if (strncmp(hwmon_buf, "inv_bmc", 7) == 0) {
            hwm_psoc = hwid;
        }
        ...
        else if (strncmp(hwmon_buf, "inv_cpld", 8) == 0) {
            hwm_cpld = hwid;
        }
    }
...
#if SINGLE_STATUS_LED
        case 1:
            status_led_red("1");    //0.5Hz
            break;
        case 2:
            status_led_red("2");    //1Hz
            break;
        case 3:
            status_led_red("3");    //2Hz
            break;
        case FAN_TBL_TOTAL:
            status_led_red("7");    //lit
            printk(KERN_ERR "[p_thread] All fans failed.\n");
            printk(KERN_ERR "[p_thread] System is going to shutdown immediately in few seconds.\n");
            break;
#else
        case 0:
            if (status_led_check_color(STATUS_FAN_LED))
                set_status_led(STATUS_LED_GRN, STATUS_FAN_LED);
                fan_led_grn = 1;
            break;
        case 1:
            set_status_led(STATUS_LED_AMBER, STATUS_FAN_LED);
            fan_led_grn = 0;
            break;
        case 2:
        case 3:
            set_status_led(STATUS_LED_RED, STATUS_FAN_LED);
            fan_led_grn = 0;
            break;
        case FAN_TBL_TOTAL:
            set_status_led(STATUS_LED_RED, STATUS_FAN_LED);
            ...
            if (strncmp(devnamep->inv_dev_attrp, PSU_ATTR_STATE, PSU_ATTR_STATE_LEN) == 0) {
                ...
                if (inventec_show_attr(state, acc_path) <= 0) {
                    ...
                    if (strncmp(psu_statep, state, strlen(state)) != 0) {
                        ...
                        SYSFS_LOG(KERN_INFO "[p_thread] %s: %s\n", psu_dev_group[i].psu_name, state);
                    }
                }
                else if (psu_voltin > PSU_VOLTIN_ACDC) {
                    /* AC PSUS */
...
#else
        set_status_led(STATUS_LED_AMBER, STATUS_PSU_LED);
        psu_led_grn = 0;
        return 1;
    }
    else {
        if (status_led_check_color(STATUS_PSU_LED))
            set_status_led(STATUS_LED_GRN, STATUS_PSU_LED);
            psu_led_grn = 1;
    }
#endif
...
#else // else SINGLE_STATUS_LED

// sys led
#define SYS_STATUS_LED_GRN_PATH     "/sys/class/hwmon/hwmon%d/device/sys_grn_led"
static char sys_status_led_grn_path[MAX_PATH_SIZE];

// fan led
#define FAN_STATUS_LED_GRN_PATH     "/sys/class/hwmon/hwmon%d/device/fan_grn_led"
#define FAN_STATUS_LED_RED_PATH     "/sys/class/hwmon/hwmon%d/device/fan_red_led"
#define FAN_STATUS_LED_AMBER_PATH   "/sys/class/hwmon/hwmon%d/device/fan_amber_led"
#define FAN_MODULE_LED_GRN1_PATH    "/sys/class/hwmon/hwmon%d/fan_led_grn1"
#define FAN_MODULE_LED_GRN2_PATH    "/sys/class/hwmon/hwmon%d/fan_led_grn2"
#define FAN_MODULE_LED_GRN3_PATH    "/sys/class/hwmon/hwmon%d/fan_led_grn3"
#define FAN_MODULE_LED_GRN4_PATH    "/sys/class/hwmon/hwmon%d/fan_led_grn4"
#define FAN_MODULE_LED_RED1_PATH    "/sys/class/hwmon/hwmon%d/fan_led_red1"
#define FAN_MODULE_LED_RED2_PATH    "/sys/class/hwmon/hwmon%d/fan_led_red2"
#define FAN_MODULE_LED_RED3_PATH    "/sys/class/hwmon/hwmon%d/fan_led_red3"
#define FAN_MODULE_LED_RED4_PATH    "/sys/class/hwmon/hwmon%d/fan_led_red4"
static char fan_status_led_grn_path[MAX_PATH_SIZE];
static char fan_status_led_red_path[MAX_PATH_SIZE];
static char fan_status_led_amber_path[MAX_PATH_SIZE];
static char fan_module_led_grn1_path[MAX_PATH_SIZE];
static char fan_module_led_grn2_path[MAX_PATH_SIZE];
static char fan_module_led_grn3_path[MAX_PATH_SIZE];
static char fan_module_led_grn4_path[MAX_PATH_SIZE];
static char fan_module_led_red1_path[MAX_PATH_SIZE];
static char fan_module_led_red2_path[MAX_PATH_SIZE];
static char fan_module_led_red3_path[MAX_PATH_SIZE];
static char fan_module_led_red4_path[MAX_PATH_SIZE];

// psu led
#define PSU_STATUS_LED_GRN_PATH     "/sys/class/hwmon/hwmon%d/device/psu_grn_led"
#define PSU_STATUS_LED_RED_PATH     "/sys/class/hwmon/hwmon%d/device/psu_red_led"
#define PSU_STATUS_LED_AMBER_PATH   "/sys/class/hwmon/hwmon%d/device/psu_amber_led"
static char psu_status_led_grn_path[MAX_PATH_SIZE];
static char psu_status_led_red_path[MAX_PATH_SIZE];
static char psu_status_led_amber_path[MAX_PATH_SIZE];
...
{
    // sys
    sprintf(&sys_status_led_grn_path[0], SYS_STATUS_LED_GRN_PATH, get_hwm_cpld());
    // fan
    sprintf(&fan_status_led_grn_path[0], FAN_STATUS_LED_GRN_PATH, get_hwm_cpld());
    sprintf(&fan_status_led_red_path[0], FAN_STATUS_LED_RED_PATH, get_hwm_cpld());
    sprintf(&fan_status_led_amber_path[0], FAN_STATUS_LED_AMBER_PATH, get_hwm_cpld());
    sprintf(&fan_module_led_grn1_path[0], FAN_MODULE_LED_GRN1_PATH, get_hwm_psoc());
    sprintf(&fan_module_led_grn2_path[0], FAN_MODULE_LED_GRN2_PATH, get_hwm_psoc());
    sprintf(&fan_module_led_grn3_path[0], FAN_MODULE_LED_GRN3_PATH, get_hwm_psoc());
    sprintf(&fan_module_led_grn4_path[0], FAN_MODULE_LED_GRN4_PATH, get_hwm_psoc());
    sprintf(&fan_module_led_red1_path[0], FAN_MODULE_LED_RED1_PATH, get_hwm_psoc());
    sprintf(&fan_module_led_red2_path[0], FAN_MODULE_LED_RED2_PATH, get_hwm_psoc());
    sprintf(&fan_module_led_red3_path[0], FAN_MODULE_LED_RED3_PATH, get_hwm_psoc());
    sprintf(&fan_module_led_red4_path[0], FAN_MODULE_LED_RED4_PATH, get_hwm_psoc());
    // psu
    sprintf(&psu_status_led_grn_path[0], PSU_STATUS_LED_GRN_PATH, get_hwm_cpld());
    sprintf(&psu_status_led_red_path[0], PSU_STATUS_LED_RED_PATH, get_hwm_cpld());
    sprintf(&psu_status_led_amber_path[0], PSU_STATUS_LED_AMBER_PATH, get_hwm_cpld());
    // diag
    sprintf(&hwmon_device_diag_path[0], HWMON_DEVICE_DIAG_PATH, get_hwm_psoc());
//    sprintf(&hwmon_device_ctrl_path[0], HWMON_DEVICE_CTRL_PATH, get_hwm_cpld());
}
...
int
status_led_check_color(const char *led_type)
{
//    char tmpbuf[MIN_ACC_SIZE];
    int ret = 0;

    if (led_type == STATUS_FAN_LED) {
        if (fan_led_grn != 1) {
            ret = 1;
        }
    }
    else if (led_type == STATUS_PSU_LED) {
        if (psu_led_grn != 1) {
            ret = 1;
        }
    }
...
/*
    if (inventec_show_attr(&tmpbuf[0], fan_status_led_grn_path) > 0) {
        if (tmpbuf[0] == STATUS_LED_GRN) {
            ret = STATUS_LED_GRN0;
        }
        if (tmpbuf[0] == STATUS_LED_RED) {
            ret = STATUS_LED_GRN1;
        }
        if (tmpbuf[0] == STATUS_LED_AMBER) {
            ret = STATUS_LED_GRN2;
        }
    }

    if (inventec_show_attr(&tmpbuf[0], fan_status_led_amber_path) > 0) {
        if (tmpbuf[0] == STATUS_LED_GRN) {
            ret = STATUS_LED_GRN0;
        }
        if (tmpbuf[0] == STATUS_LED_RED) {
    ...
    if (inventec_show_attr(&tmpbuf[0], fan_status_led_red_path) > 0) {
        if (tmpbuf[0] == STATUS_LED_GRN) {
            ret = STATUS_LED_GRN0;
        }
        if (tmpbuf[0] == STATUS_LED_RED) {
...
#endif //end of SINGLE_STATUS_LED
...
status_led_red(const char *freq)
{
    ...
    ret = inventec_store_attr(STATUS_LED_OFF, 1, &status_led_grn_path[0]);
    if (ret < 0) {
        return ret;
    }
...
status_led_grn(const char *freq)
{
    ...
    ret = inventec_store_attr(STATUS_LED_OFF, 1, &status_led_red_path[0]);
    if (ret < 0) {
        return ret;
    }
...
ssize_t
set_status_led(const char *color, const char *led_type)
{
    ssize_t ret;
    char *grn_path;
    char *red_path;
    ...
    if (led_type == STATUS_FAN_LED) {
        grn_path = &fan_status_led_grn_path[0];
        red_path = &fan_status_led_red_path[0];
        amber_path = &fan_status_led_amber_path[0];
    }
    else if (led_type == STATUS_PSU_LED) {
        grn_path = &psu_status_led_grn_path[0];
        red_path = &psu_status_led_red_path[0];
        amber_path = &psu_status_led_amber_path[0];
    }
    else if (led_type == STATUS_SYS_LED) {
        grn_path = &sys_status_led_grn_path[0];
        if ((ret = inventec_store_attr(STATUS_LED_GRN, 1, grn_path)) < 0) { return ret; }
    }
    else {
        return -1;
    }

    if (color == STATUS_LED_GRN) {
        if ((ret = inventec_store_attr(STATUS_LED_OFF, 1, red_path)) < 0) { return ret; }
        if ((ret = inventec_store_attr(STATUS_LED_OFF, 1, amber_path)) < 0) { return ret; }
        if ((ret = inventec_store_attr(STATUS_LED_GRN, 1, grn_path)) < 0) { return ret; }
    }
    else if (color == STATUS_LED_AMBER) {
        if ((ret = inventec_store_attr(STATUS_LED_OFF, 1, grn_path)) < 0) { return ret; }
        if ((ret = inventec_store_attr(STATUS_LED_OFF, 1, red_path)) < 0) { return ret; }
        if ((ret = inventec_store_attr(STATUS_LED_AMBER, 1, amber_path)) < 0) { return ret; }
    }
    else if (color == STATUS_LED_RED) {
        if ((ret = inventec_store_attr(STATUS_LED_OFF, 1, grn_path)) < 0) { return ret; }
        if ((ret = inventec_store_attr(STATUS_LED_OFF, 1, amber_path)) < 0) { return ret; }
        if ((ret = inventec_store_attr(STATUS_LED_RED, 1, red_path)) < 0) { return ret; }
    }
    else {
        ret = -1;
    }
...
static int thread_fn(void *unused)
{
    ...
    status_led_grn("7");
#else
    set_status_led(STATUS_LED_GRN, STATUS_SYS_LED);
    set_status_led(STATUS_LED_GRN, STATUS_FAN_LED);
    set_status_led(STATUS_LED_GRN, STATUS_PSU_LED);
#endif
    psu_get_voltin();
    ...
    printk(KERN_INFO "[p_thread] %s/%d: Thread Starting\n",__func__,__LINE__);
    while (1)
    {
...
#if SINGLE_STATUS_LED
        /* check fan and psu state */
        if (fans_control() > 0) {
            psus_control(1);
            continue;
        }
        else if (psus_control(0) > 0) {
            continue;
        }

        /* status led red, change it to green */
        if (status_led_check_color() != STATUS_LED_GRN7) {
            status_led_grn("7");
        }
#else
        fans_control();

...
static int inv_i2c_smbus_read_block_data(struct i2c_client *client, u8 offset, int len, u8 *buf)
{
    int ret = 0;
    int i = 0;
...
_common_update_string_attr(struct transvr_obj_s *self,
    ...
    #else
    err = inv_i2c_smbus_read_block_data(self->i2c_client_p, offset, len, buf);
    if (err < 0){
            emsg = "I2C R/W fail!";
            goto err_common_update_string_attr;
        }
...
_sfp_detect_if_type(struct transvr_obj_s* self,
    ...
    switch (self->chipset_type) {
        ...
        case CHIP_TYPE_COTTONWOOD_LITE:

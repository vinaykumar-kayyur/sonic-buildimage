...
extern struct mutex ipmi_mutex;
extern struct mutex ipmi2_mutex;
extern int start_ipmi_command (char NetFn, char cmd, char *data, int data_length,char *result, int *result_length);
			 ...
			 char *buf)
{
	...
	if (attr->index < 4){
		status = psoc_read8(FAN_GPI_OFFSET);		
	}
	else {
		status = psoc_read8(FAN_GPI2_OFFSET);
		index-=4;
	}
	...
	return sprintf(buf, "%d:%s\n", type, fantype_str[type]);
}

ssize_t show_switch_tmp(struct device *dev, struct device_attribute *da,
			 char *buf)
{
	u16 status=0;
	u16 temp = 0;

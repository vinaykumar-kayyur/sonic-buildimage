...
def new_i2c_devices(new_device_list):
    for (driver, addr, bus_number) in new_device_list:
        for i in range(0, 10):
            if new_i2c_device(driver, addr, bus_number) != True:
                time.sleep(1)
                continue
            else:
                break
...
def new_sfp_devices(sfp_bus_list, driver):
    for bus_number in sfp_bus_list:
        for i in range(0, 10):
            if new_i2c_device(driver, 0x50, bus_number) != True:
                time.sleep(1)
                continue
            else:
                break
...
def device_install():
    ...
    for i in chain(range(1, 11), range(21, 31)):
        path = '/sys/bus/i2c/devices/12-0062/module_reset_'+str(i)
        if write_txt_file(path, '0') is not True:
            return 1

    for i in chain(range(11, 21), range(31, 41)):
        path = '/sys/bus/i2c/devices/13-0063/module_reset_'+str(i)
        if write_txt_file(path, '0') is not True:
            return 1

    for i in range(41, 54):
        path = '/sys/bus/i2c/devices/20-0064/module_reset_'+str(i)
        if write_txt_file(path, '0') is not True:
            return 1
    ...
    port_bus_list = qsfp_bus_list + sfp_bus_list

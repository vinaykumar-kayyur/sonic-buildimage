...
args = []
ALL_DEVICE = {}
DEVICE_NO = {'led': 8, 'fan': 3, 'thermal': 3, 'psu': 2, 'sfp': 6}
...
hwmon_types = {
    'led': ['diag', 'pri', 'poe', 'stk1', 'stk2', 'fan', 'psu1', 'psu2']
}
...
i2c_bus = {
    'fan': ['3-0060'],
    'thermal': ['14-0048', '25-004a', '24-004b'],
    'psu': ['10-0050', '11-0051'],
    'sfp': ['-0050']
}
i2c_nodes = {
    'fan': ['/fan_present_{}', '/fan{}_input'],
...
cpld_set = [
    'i2cset -y -f 3 0x60 0x2a 0xff', 'i2cset -y -f 3 0x60 0x2b 0xff',
...
def show_eeprom_help():
    cmd = sys.argv[0].split("/")[-1] + " " + args[0]
    print "    use \"" + cmd + " 49-54 \" to dump sfp# eeprom"
    sys.exit(0)
...
kos = [
    'depmod -ae', 'modprobe i2c_dev',
    'modprobe i2c_mux_pca954x force_deselect_on_exit=1', 'modprobe ym2651y',
    'modprobe x86_64_accton_as4630_54pe_cpld',
    'modprobe x86_64_accton_as4630_54pe_leds',
    'modprobe x86_64_accton_as4630_54pe_psu', 'modprobe optoe'
]
...
def device_install():
    ...
    for i in range(0, len(sfp_map)):
        ...
        status, output = log_os_system(
            "echo " + str(opt) + " 0x50 > /sys/bus/i2c/devices/i2c-" +
        ...
        status, output = log_os_system(
            "echo port" + str(i + 49) + " > /sys/bus/i2c/devices/" +
...
def device_uninstall():
    ...
    for i in range(0, len(sfp_map)):
        target = "/sys/bus/i2c/devices/i2c-" + str(
            sfp_map[i]) + "/delete_device"
            ...
            for j in range(0, len(nodes)):
                if 'fan' == key:
                    for k in range(0, DEVICE_NO[key]):
                        node = key + str(k + 1)
                        path = i2c_prefix + buses[i] + nodes[j].format(k + 1)
                        my_log(node + ": " + path)
                        ALL_DEVICE[key][node].append(path)
                elif 'sfp' == key:
                    for k in range(0, DEVICE_NO[key]):
                        node = key + str(k + 1)
                        path = i2c_prefix + "3-0060/" + nodes[j] + str(
                            k + 1 + SFP_START)
                        my_log(node + ": " + path)
                        ALL_DEVICE[key][node].append(path)
                else:
...
def set_device(args):
    ...
    if args[0] == 'led':
        ...
        if int(args[1]) > PORT_NUM or int(args[1]) <= SFP_START:
            show_set_help()
            return
        if len(args) < 2:
            show_set_help()
        ...
        for i in range(0, len(ALL_DEVICE[args[0]])):
            pn = int(args[1]) - SFP_START
            for j in ALL_DEVICE[args[0]][args[0] + str(pn)]:

...
#define DRV_NAME        "as5912_54x_psu"
...
static int as5912_54x_psu_block_read(struct i2c_client *client, u8 command, u8 *data,int data_len);
static int as5912_54x_psu_model_name_get(
    struct device *dev, char *buf);
static int as5912_54x_psu_serial_number_get(
                                  ...
                                  char *buf)
{
    ...
    i = as5912_54x_psu_model_name_get(dev, buf);
    if ( i < 0) {
        return -ENXIO;
    }

    if (as5912_54x_psu_serial_number_get(dev, i, buf) < 0) {
        return -ENXIO;
    }
                               ...
                               char *buf)
{
    ...
    if (as5912_54x_psu_model_name_get(dev, buf) < 0) {
        return -ENXIO;
    }
...
static int as5912_54x_psu_block_read(struct i2c_client *client,
...
static struct as5912_54x_psu_data *as5912_54x_psu_update_device(struct device *dev)
{
    ...
    if (time_after(jiffies, data->last_updated + UPDATE_PERIOD)
        ...
        status = as5912_54x_psu_block_read(client, 0,
...
MODULE_DESCRIPTION("accton as5912_54x_psu driver");
MODULE_LICENSE("GPL");

...
struct led_list_s {
    ...
    {TYPE_DIAG, "as5912_54x_diag", 0x41, CPLD_I2C_ADDR},
    {TYPE_LOC,  "as5912_54x_loc", 0x40, CPLD_I2C_ADDR}
};
...
static int pdata_init(struct accton_as5912_54x_led_data  *data) {
    int i;
...
static int accton_as5912_54x_led_read_value(u8 slave, u8 reg)
{
    return accton_i2c_cpld_read(slave, reg);
}

static int accton_as5912_54x_led_write_value(u8 slave, u8 reg, u8 value)
{
...
static void accton_as5912_54x_led_update(void)
{
    ...
    if (time_after(jiffies, ledctl->last_updated + HZ + HZ / 2)
        ...
        for (i = 0; i < ARRAY_SIZE(ledctl->reg_val); i++) {
            ...
            status = accton_as5912_54x_led_read_value(addr, offset);
            if (status < 0) {
                ledctl->valid = 0;
                                       ...
                                       enum led_type type)
{
    ...
    reg_val = accton_as5912_54x_led_read_value(addr, offset);
    if (reg_val < 0) {
        dev_dbg(&ledctl->pdev->dev, "reg %d, err %d\n", offset, reg_val);
        goto exit;
    }
    ...
    accton_as5912_54x_led_write_value(addr, offset, reg_val);
                         ...
                         enum led_brightness led_light_mode)
{
    ...
    accton_as5912_54x_led_set(cdev, led_light_mode, type);
}
...
static struct led_classdev accton_as5912_54x_leds[TYPE_MAX] = {
    [TYPE_DIAG] = {
...
static int accton_as5912_54x_led_suspend(struct platform_device *dev,
        pm_message_t state)
{
    int i = 0;

    for (i = 0; i < ARRAY_SIZE(accton_as5912_54x_leds); i++) {
        led_classdev_suspend(&accton_as5912_54x_leds[i]);
    }

    return 0;
}

static int accton_as5912_54x_led_resume(struct platform_device *dev)
{
    int i = 0;

    for (i = 0; i < ARRAY_SIZE(accton_as5912_54x_leds); i++) {
        led_classdev_resume(&accton_as5912_54x_leds[i]);
    }
...
static struct platform_driver accton_as5912_54x_led_driver = {
    ...
    .suspend    = accton_as5912_54x_led_suspend,
    .resume     = accton_as5912_54x_led_resume,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
module_init(accton_as5912_54x_led_init);

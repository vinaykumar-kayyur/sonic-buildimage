...
#define DRVNAME "as7535_32xb_led"
...
static ssize_t show_led(struct device *dev, struct device_attribute *attr,
			char *buf);
static int as7535_32xb_led_probe(struct platform_device *pdev);
static int as7535_32xb_led_remove(struct platform_device *pdev);
...
struct as7535_32xb_led_data *data = NULL;

static struct platform_driver as7535_32xb_led_driver = {
	.probe = as7535_32xb_led_probe,
	.remove = as7535_32xb_led_remove,
	.driver = {
		.name = DRVNAME,
		.owner = THIS_MODULE,
	},
...
enum as7535_32xb_led_sysfs_attrs {
	LED_LOC,
	LED_DIAG,
	LED_GNSS,
	LED_FAN,
...
static SENSOR_DEVICE_ATTR(led_diag, S_IWUSR | S_IRUGO, show_led, set_led,
							LED_DIAG);
static SENSOR_DEVICE_ATTR(led_gnss, S_IWUSR | S_IRUGO, show_led, set_led,
							LED_GNSS);
...
static struct attribute *as7535_32xb_led_attributes[] = {
...
static const struct attribute_group as7535_32xb_led_group = {
	.attrs = as7535_32xb_led_attributes,
};
...
static struct as7535_32xb_led_data *as7535_32xb_led_update_device(void)
{
	int status = 0;
							...
							char *buf)
{
	...
	data = as7535_32xb_led_update_device();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int as7535_32xb_led_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7535_32xb_led_group);
	if (status)
		goto exit;
...
static int as7535_32xb_led_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &as7535_32xb_led_group);

	return 0;
}

static int __init as7535_32xb_led_init(void)
{
	int ret;
	...
	ret = platform_driver_register(&as7535_32xb_led_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as7535_32xb_led_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7535_32xb_led_exit(void)
{
...
MODULE_DESCRIPTION("as7535_32xb_led driver");
MODULE_LICENSE("GPL");

module_init(as7535_32xb_led_init);
module_exit(as7535_32xb_led_exit);

...
#define DRVNAME "as7535_32xb_psu"
...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
							char *buf);
static int as7535_32xb_psu_probe(struct platform_device *pdev);
static int as7535_32xb_psu_remove(struct platform_device *pdev);
...
struct as7535_32xb_psu_data *data = NULL;

static struct platform_driver as7535_32xb_psu_driver = {
	.probe = as7535_32xb_psu_probe,
	.remove = as7535_32xb_psu_remove,
	.driver = {
		.name = DRVNAME,
		.owner = THIS_MODULE,
	},
...
enum as7535_32xb_psu_sysfs_attrs {
	/* psu attributes */
	PSU_ATTR(1),
	PSU_ATTR(2),
	NUM_OF_PSU_ATTR,
...
static struct attribute *as7535_32xb_psu_attributes[] = {
	/* psu attributes */
	DECLARE_PSU_ATTR(1),
	DECLARE_PSU_ATTR(2),
	NULL
...
static const struct attribute_group as7535_32xb_psu_group = {
	.attrs = as7535_32xb_psu_attributes,
};
...
static struct as7535_32xb_psu_data *as7535_32xb_psu_update_device(struct device_attribute *da)
{
							...
							char *buf)
{
	...
	data = as7535_32xb_psu_update_device(da);
	if (!data->valid[pid]) {
		error = -EIO;
		goto exit;
	}
...
static int as7535_32xb_psu_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7535_32xb_psu_group);
	if (status)
		return status;
...
static int as7535_32xb_psu_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &as7535_32xb_psu_group);
	return 0;
}

static int __init as7535_32xb_psu_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct as7535_32xb_psu_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as7535_32xb_psu_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as7535_32xb_psu_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7535_32xb_psu_exit(void)
{
...
MODULE_DESCRIPTION("as7535_32xb_psu driver");
MODULE_LICENSE("GPL");

module_init(as7535_32xb_psu_init);

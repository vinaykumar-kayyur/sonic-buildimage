...
#define DRVNAME "as7535_32xb_fan"
...
struct as7535_32xb_fan_data *data = NULL;

static struct platform_driver as7535_32xb_fan_driver = {
	.probe = as7535_32xb_fan_probe,
	.remove = as7535_32xb_fan_remove,
	.driver = {
		.name = DRVNAME,
		.owner = THIS_MODULE,
	},
...
static struct attribute *as7535_32xb_fan_attributes[] = {
	/* fan attributes */
	DECLARE_FAN_ATTR(1),
	DECLARE_FAN_ATTR(2),
	DECLARE_FAN_ATTR(3),
...
static const struct attribute_group as7535_32xb_fan_group = {
	.attrs = as7535_32xb_fan_attributes,
};
...
static struct as7535_32xb_fan_data *as7535_32xb_fan_update_device(void)
{
	int status = 0;
...
static struct as7535_32xb_fan_data *as7535_32xb_fan_update_cpld_ver(void)
{
	int status = 0;
								...
								char *buf)
{
	...
	data = as7535_32xb_fan_update_cpld_ver();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int as7535_32xb_fan_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7535_32xb_fan_group);
	if (status)
		return status;
...
static int as7535_32xb_fan_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &as7535_32xb_fan_group);

	return 0;
}

static int __init as7535_32xb_fan_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct as7535_32xb_fan_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as7535_32xb_fan_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as7535_32xb_fan_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7535_32xb_fan_exit(void)
{
...
MODULE_DESCRIPTION("as7535_32xb_fan driver");
MODULE_LICENSE("GPL");

module_init(as7535_32xb_fan_init);
module_exit(as7535_32xb_fan_exit);

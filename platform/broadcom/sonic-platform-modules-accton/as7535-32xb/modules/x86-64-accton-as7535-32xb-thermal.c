...
static int as7535_32xb_thermal_remove(struct platform_device *pdev);
...
struct as7535_32xb_thermal_data {
...
static s8 temp_max_alarm[THERMAL_COUNT] = { 79, 89, 102, 92, 85, 85 };
static s8 temp_max[THERMAL_COUNT] = { 74, 84, 97, 87, 80, 80 };
static s8 temp_min[THERMAL_COUNT] = { -45, -45, -45, -45, -45, -45 };
static s8 temp_min_alarm[THERMAL_COUNT] = { -50, -50, -50, -50, -50, -50 };

struct as7535_32xb_thermal_data *data = NULL;

static struct platform_driver as7535_32xb_thermal_driver = {
	.probe = as7535_32xb_thermal_probe,
...
enum as7535_32xb_thermal_sysfs_attrs {
	TEMP1_INPUT, // 0x4B
...
#define DECLARE_THERMAL_SENSOR_DEVICE_ATTR(index) \
	...
	static SENSOR_DEVICE_ATTR(temp##index##_max_alarm, S_IRUGO, show_threshold,\
...
static const struct attribute_group as7535_32xb_thermal_group = {
	.attrs = as7535_32xb_thermal_attributes,
};
...
static int as7535_32xb_thermal_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7535_32xb_thermal_group);
	if (status)
		return status;
...
static int as7535_32xb_thermal_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &as7535_32xb_thermal_group);
	return 0;
}

static int __init as7535_32xb_thermal_init(void)
{
	int ret;

	data = kzalloc(sizeof(struct as7535_32xb_thermal_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as7535_32xb_thermal_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as7535_32xb_thermal_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7535_32xb_thermal_exit(void)
{
...
MODULE_DESCRIPTION("as7535_32xb_thermal driver");
MODULE_LICENSE("GPL");

module_init(as7535_32xb_thermal_init);
module_exit(as7535_32xb_thermal_exit);

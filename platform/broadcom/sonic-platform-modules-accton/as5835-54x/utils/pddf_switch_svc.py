...
def stop_platform_svc():
    ...
    status, output = commands.getstatusoutput("systemctl stop as5835-54x-platform-monitor-fan.service")
    if status:
        print "Stop as5835-54x-platform-fan.service failed %d"%status
        return False
    
    status, output = commands.getstatusoutput("systemctl stop as5835-54x-platform-monitor-psu.service")
    if status:
        print "Stop as5835-54x-platform-psu.service failed %d"%status
        return False
    
    status, output = commands.getstatusoutput("systemctl stop as5835-54x-platform-monitor.service")
    if status:
        print "Stop as5835-54x-platform-init.service failed %d"%status
        return False
    status, output = commands.getstatusoutput("systemctl disable as5835-54x-platform-monitor.service")
    if status:
        print "Disable as5835-54x-platform-monitor.service failed %d"%status
        return False
    
    status, output = commands.getstatusoutput("/usr/local/bin/accton_as5835_54x_util.py clean")
    if status:
...
def start_platform_svc():
    status, output = commands.getstatusoutput("/usr/local/bin/accton_as5835_54x_util.py install")
    if status:
        print "accton_as5835_54x_util.py install command failed %d"%status
        return False

    status, output = commands.getstatusoutput("systemctl enable as5835-54x-platform-monitor.service")
    if status:
        print "Enable as5835-54x-platform-monitor.service failed %d"%status
        return False
    status, output = commands.getstatusoutput("systemctl start as5835-54x-platform-monitor-fan.service")
    if status:
        print "Start as5835-54x-platform-monitor-fan.service failed %d"%status
        return False
        
    status, output = commands.getstatusoutput("systemctl start as5835-54x-platform-monitor-psu.service")
    if status:

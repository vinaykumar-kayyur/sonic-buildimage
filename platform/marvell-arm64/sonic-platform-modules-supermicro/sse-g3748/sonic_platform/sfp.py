...
class Sfp(SfpBase):
    ...
    """
    SSE-G3748 platform-specific Sfp class
    """
    ...
    def _dom_capability_detect(self):
        if self.sfp_type == "SFP":
            ...
            if sfp_dom_capability_raw is not None:
                ...
                #sonic_logger.log_warning("sfp sfp_dom_capability={}".format(sfp_dom_capability))
                ...
                if self.dom_supported:
                    ...
                    if sfp_dom_capability & 0x20 != 0:
                        ...
                        self.calibration = 0
                    #sonic_logger.log_warning("sfp dom_supported is true");
                else:
                    self.dom_temp_supported = False
                    self.dom_volt_supported = False
                    self.dom_rx_power_supported = False
                    self.dom_tx_power_supported = False
                    self.calibration = 0
                    #sonic_logger.log_warning("sfp dom_supported is false");
                self.dom_tx_disable_supported = (
                    int(sfp_dom_capability_raw[1], 16) & 0x40 != 0)
                #sonic_logger.log_warning("sfp dom_tx_disable_supported is {}".format(self.dom_tx_disable_supported))
        else:
            self.dom_supported = False
            self.dom_temp_supported = False
            self.dom_volt_supported = False
    ...
    def get_rx_los(self):
        ...
        #sonic_logger.log_warning("sfp get_rx_los: {} index={}".format(rx_los_list, self.index))
        return rx_los_list
    ...
    def get_tx_fault(self):
        ...
        if dom_channel_monitor_raw is not None:
            ...
            return None
        #sonic_logger.log_warning("sfp get_tx_fault: {} index={}".format(tx_fault_list, self.index))
    ...
    def get_tx_disable(self):
        ...
        #sonic_logger.log_warning("sfp get_tx_disable: {} index={}".format(tx_disable_list, self.index))
    ...
    def get_power_override(self):
        ...
        if self.sfp_type == COPPER_TYPE:
            return False
        if self.sfp_type == SFP_TYPE:
            #sonic_logger.log_warning("sfp get_power_override index={}".format(self.index))
            return False
    ...
    def tx_disable(self, tx_disable):
        ...
        if smbus_present == 0:  # if called from sfputil outside of pmon
            regin = reg_in[self.index-SFP_PORT_START]
            cmdstatus, register = cmd.getstatusoutput('sudo i2cget -y 1 0x22 %d' % regin )
            if cmdstatus:
            ...
            bus = smbus.SMBus(1)
            DEVICE_ADDRESS = 0x22
            DEVICE_REG = reg_in[self.index-SFP_PORT_START]
        ...
        if tx_disable == True:
            setbits = register | tx_dis_bit[self.index-SFP_PORT_START]
        else:
            setbits = register & tx_en_bit[self.index-SFP_PORT_START]

        if smbus_present == 0:  # if called from sfputil outside of pmon
            """
            cmdstatus, output = cmd.getstatusoutput('sudo i2cset -y -m 0x0f 1 0x22 0x8 %d' % setbits)
            """
            regout = reg_out[self.index-SFP_PORT_START]
            cmdstatus, output = cmd.getstatusoutput('sudo i2cset -y 1 0x22 %d %d' % regout % setbits)
            if cmdstatus:
            ...
            bus = smbus.SMBus(1)
            DEVICE_ADDRESS = 0x22
            DEVICE_REG = reg_out[self.index-SFP_PORT_START]
    ...
    def get_presence(self):
        ...
        if ((self.index-SFP_PORT_START)%2) == 1:

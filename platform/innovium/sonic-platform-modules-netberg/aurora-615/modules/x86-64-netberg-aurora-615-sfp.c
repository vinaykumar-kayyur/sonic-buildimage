...
#include "x86-64-netberg-aurora-615-sfp.h"

/* i2c_client Declaration */
extern struct i2c_client *Netberg_CPLD_31_client; //0x31 for Port 01-32
extern struct i2c_client *Netberg_CPLD_32_client; //0x32 for Port 33-48
...
ssize_t sfp_tx_enable_all_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, SFP_41_48_TX_ENABLE_REG); //41-48
        result = status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, SFP_33_40_TX_ENABLE_REG); //33-40
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_25_32_TX_ENABLE_REG); //25-32
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_17_24_TX_ENABLE_REG); //17-24
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_9_16_TX_ENABLE_REG);  //9-16
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_1_8_TX_ENABLE_REG);   //1-8
        result = (result << 8) | status;
...
ssize_t sfp_tx_enable_all_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (attr->index == SFP_TX_ENABLE_ALL)
    {
        ...
        result += i2c_smbus_write_byte_data(Netberg_CPLD_31_client, SFP_1_8_TX_ENABLE_REG, value);
        result += i2c_smbus_write_byte_data(Netberg_CPLD_31_client, SFP_9_16_TX_ENABLE_REG, value);
        result += i2c_smbus_write_byte_data(Netberg_CPLD_31_client, SFP_17_24_TX_ENABLE_REG, value);
        result += i2c_smbus_write_byte_data(Netberg_CPLD_31_client, SFP_25_32_TX_ENABLE_REG, value);
        result += i2c_smbus_write_byte_data(Netberg_CPLD_32_client, SFP_33_40_TX_ENABLE_REG, value);
        result += i2c_smbus_write_byte_data(Netberg_CPLD_32_client, SFP_41_48_TX_ENABLE_REG, value);
...
ssize_t sfp_tx_enable_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, sfp_tx_enable_regs[port_index][0]);
    }
    else if (port_index >= 33 && port_index <= 48)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, sfp_tx_enable_regs[port_index][0]);
    }
...
ssize_t sfp_rx_loss_all_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, SFP_41_48_RX_LOSS_REG); //41-48
        result = status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, SFP_33_40_RX_LOSS_REG); //33-40
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_25_32_RX_LOSS_REG); //25-32
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_17_24_RX_LOSS_REG); //17-24
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_9_16_RX_LOSS_REG);  //9-16
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_1_8_RX_LOSS_REG);   //1-8
        result = (result << 8) | status;
        result = (~(result)) & 0xFFFFFFFFFFFF;
        sprintf(buf, "%s0x%llx\n", buf, result);
    }
...
ssize_t sfp_rx_loss_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (port_index >= 1 && port_index <= 32)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, sfp_rx_loss[port_index][0]);
    }
    else if (port_index >= 33 && port_index <= 48)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, sfp_rx_loss[port_index][0]);
    }
...
ssize_t sfp_present_all_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, SFP_41_48_PRESENT_REG); //41-48
        result = status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, SFP_33_40_PRESENT_REG); //33-40
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_25_32_PRESENT_REG); //25-32
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_17_24_PRESENT_REG); //17-24
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_9_16_PRESENT_REG);  //9-16
        result = (result << 8) | status;
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, SFP_1_8_PRESENT_REG);   //1-8
        result = (result << 8) | status;
        result = (~(result)) & 0xFFFFFFFFFFFF;
        sprintf(buf, "%s0x%llx\n", buf, result);
    }
...
ssize_t sfp_present_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (port_index >= 1 && port_index <= 32)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, sfp_present_regs[port_index][0]);
    }
    else if (port_index >= 33 && port_index <= 48)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, sfp_present_regs[port_index][0]);
    }
...
ssize_t sfp_rx_loss_int_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, sfp_rx_loss_int_regs[int_index][0]);
    }
    else if (int_index >= 5 && int_index <= 6)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, sfp_rx_loss_int_regs[int_index][0]);
    }
...
ssize_t sfp_rx_loss_int_mask_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (int_index >= 1 && int_index <= 4)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, sfp_rx_loss_int_mask_regs[int_index][0]);
    }
    else if (int_index >= 5 && int_index <= 6)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, sfp_rx_loss_int_mask_regs[int_index][0]);
    }
...
ssize_t sfp_present_int_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (int_index >= 1 && int_index <= 4)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, sfp_present_int_regs[int_index][0]);
    }
    else if (int_index >= 5 && int_index <= 6)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, sfp_present_int_regs[int_index][0]);
    }
...
ssize_t sfp_present_int_mask_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (int_index >= 1 && int_index <= 4)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_31_client, sfp_present_int_mask_regs[int_index][0]);
    }
    else if (int_index >= 5 && int_index <= 6)
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_32_client, sfp_present_int_mask_regs[int_index][0]);
    }

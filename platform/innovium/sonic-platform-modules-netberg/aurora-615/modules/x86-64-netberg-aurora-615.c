...
#if (defined THERMAL_WANTED)|| (defined ASPEED_BMC_WANTED)
...
static ssize_t thermal_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    u8 status = -EPERM;
    u8 res = 0x1;
    int i;
...
#ifdef THERMAL_WANTED
static ssize_t thermal_temp_get(struct device *dev, struct device_attribute *da, char *buf)
{
    u8 status = -EPERM;
    ...
    i = attr->index;
    debug_print((KERN_DEBUG "DEBUG : thermal_temp_get %d\n", i));
    mutex_lock(&Switch_2_data->update_lock);
    mutex_lock(&Sensor_data->update_lock);
    debug_print((KERN_DEBUG "DEBUG : thermal_temp_get mutex_lock\n"));
    sprintf(buf, "");
    if (attr->index == SENSOR_TEMP)
    {
        ...
        if(channel_status < 0)
        {
            printk(KERN_ALERT "ERROR: thermal_temp_get set channel 2 FAILED\n");
        }
        ...
        if(channel_status < 0)
        {
            printk(KERN_ALERT "ERROR: thermal_temp_get set channel 3 FAILED\n");
        }
        status = i2c_smbus_read_byte_data(Netberg_Sensor_client, 0X0);
        ...
        if(channel_status < 0)
        {
            printk(KERN_ALERT "ERROR: thermal_temp_get set channel 4 FAILED\n");
        }
        status = i2c_smbus_read_byte_data(Netberg_Sensor_client, 0X0);
        ...
        if(channel_status < 0)
        {
            printk(KERN_ALERT "ERROR: thermal_temp_get set channel 1 FAILED\n");
        }
    ...
    if(channel_status < 0)
    {
        printk(KERN_ALERT "ERROR: thermal_temp_get reset channel FAILED\n");
    }
...
static ssize_t thermal_mask_get(struct device *dev, struct device_attribute *da, char *buf)
{
    u8 status = -EPERM;
    u8 res = 0x1;
    int i;
...
static ssize_t thermal_mask_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    u8 status = -EPERM;
    u8 value  = -EPERM;
    u8 result  = -EPERM;
    ...
    mutex_lock(&data->update_lock);
    debug_print((KERN_DEBUG "DEBUG : thermal_mask_set mutex_lock\n"));
    i = simple_strtol(buf, NULL, 10);
    debug_print((KERN_DEBUG "DEBUG : thermal_mask_set input %d\n", i));
    status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xc1);
    debug_print((KERN_DEBUG "DEBUG : thermal_mask_set status = %x\n",status));
    if (attr->index == 1)
    {
        if (i == TURN_ON)
        {
        ...
        {
            printk(KERN_ALERT "thermal_mask_set set wrong value\n");
        }
    }
    else if (attr->index == 2)
    {
    ...
    debug_print((KERN_DEBUG "DEBUG : thermal_mask_set %d value = %x\n", attr->index, value));
    result = i2c_smbus_write_byte_data(NBA_615_i2c_client, 0xc1, value);
    debug_print((KERN_DEBUG "DEBUG : thermal_mask_set %d result = %x\n", attr->index,result));
    if (result < 0)
    {
        printk(KERN_ALERT "ERROR: thermal_mask_set %d FAILED!\n", attr->index);
    }
    else
    {
        debug_print((KERN_DEBUG "thermal_mask_set %d : %x\n", attr->index, value));
    }
    mutex_unlock(&data->update_lock);
    debug_print((KERN_DEBUG "DEBUG : mutex_unlock\n"));
    return count;

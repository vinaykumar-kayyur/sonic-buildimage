...
kos = [
    'depmod -a',
    'modprobe i2c_dev',
    'modprobe x86-64-cameo-esqc610-56sq',
...
def hw_adjustment():
    ...
    if hw_ver == 0x06:
        I2C_SWITCH_LIST['PCA9548_0x71_7']['i2caddr'] = '0x74'
        
        

def set_led_control():
    cmd = "echo 1 > /sys/class/hwmon/hwmon2/device/ESQC610_LED/led_ctrl"
    status, output = log_os_system(cmd, 1)
    if status:
        print output
...
def get_hw_version():
    value = ''
    filePath = '/sys/class/hwmon/hwmon2/device/ESQC610_SYS/hw_version'
...
def bmc_is_exist():
    value = ''
    bmc_filePath = '/sys/class/hwmon/hwmon2/device/ESQC610_BMC/bmc_present'

...
class Sfp(SfpBase):
    ...
    def __init__(self, sfp_index=0, sfp_name=None):
        ...
        self._name = sfputil_helper.logical[sfp_index] if sfp_name is None else sfp_name
    ...
    def _get_path_to_port_config_file(self):
        ...
        host_platform_path = "/".join([host_platform_root_path,
                                       self._api_helper.get_platform()])
        hwsku_path = "/".join([host_platform_path, self._api_helper.get_hwsku()]) \
    ...
    def _read_eeprom_specific_bytes(self, offset, num_bytes):
        ...
        eeprom_bytestream = self._api_helper.read_eeprom_qsfp_file(self._port_num, EEPROM_ADDRESS)
        eeprom_raw = []
        # try:
        #     eeprom = open(
        ...
        raw = eeprom_bytestream[offset:(offset+num_bytes)]
    ...
    def _detect_sfp_type(self):
        ...
        if eeprom_raw:
            if eeprom_raw[0] in SFP_TYPE_CODE_LIST:
                ...
                self.sfp_type = sfp_type
                log_error("unable to fetch sfp type for sfp index {}".format(self._index+1))
        else:
            log_error("unable to fetch sfp type for sfp index {}".format(self._index+1))
            self.sfp_type = sfp_type
        # print(self.sfp_type, "sfp type")

    def _get_eeprom_path(self):
        # port_to_i2c_mapping = SFP_I2C_START + (self._port_num-1)//4 + self._port_num
    ...
    def get_rx_los(self):
        ...
        Returns:
            A List of Boolean, True if SFP has RX LOS, False if not.
            for QSFP it is False
        """
        return [False]
    ...
    def reset(self):
        """
        Reset QSFP and return all user module settings to their default state.
        Returns:
    ...
    def tx_disable_channel(self, channel, disable):
        """
        Sets the tx_disable for specified QSFP channels
        Args:
        ...
        Returns:
            A boolean, True if successful, False if not 
            Note: For QSFP tx_disable is not done via plugin
        """
        return False
